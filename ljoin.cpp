...
void Query () { 
	...
	CollectionType *collType1 = dynamic_cast<CollectionType*>(datasetInfo1->exprType);
	...
	RecordAttribute* d_key1 = new RecordAttribute(1, relName1, key_str, new IntType(), false); //recType1->getArg(key_str);
	std::cout << d_key1->getType() << std::endl;
	int attrNo = 1;
	const ExpressionType* recArgType = d_key1->getOriginalType();
	RecordAttribute* d_key_block1 = new RecordAttribute(attrNo, relName1, key_str, recArgType, true);
	...
	Plugin* pg1 = new ScanToBlockSMPlugin(ctx, *path1, *recType1, expr_projections_ptr1);
	...
	RawOperator* mml1 =  new MemMoveLocalTo(scan1, ctx, expr_projections_block_ptr1, 4);
	scan1->setParent(mml1);

	RawOperator* mmd1 =  new MemMoveDevice(mml1, ctx, expr_projections_block_ptr1, 4, false);
	mml1->setParent(mmd1);
	...
	expressions::Expression* e1 = new expressions::InputArgument(recType1, -1, atts1);
	expressions::Expression* e11 = new expressions::RecordProjection(e1, *d_key1);
	expressions::Expression* e3 = new expressions::InputArgument(recType1, -1, atts1);
	...
	expressions::Expression* pred1 = new expressions::LtExpression(new BoolType(), lhs1, rhs1);
	...
	CollectionType *collType2 = dynamic_cast<CollectionType*>(datasetInfo2->exprType);
	...
	RecordAttribute* d_key2 = new RecordAttribute(1, relName2, key_str, new IntType(), false); //2recType1->getArg(key_str);
	std::cout << d_key2->getType() << std::endl;
	int attrNo2 = 1;
	const ExpressionType* recArgType2 = d_key2->getOriginalType();
	RecordAttribute* d_key_block2 = new RecordAttribute(attrNo2, relName2, key_str, recArgType2, true);
	...
	Plugin* pg2 = new ScanToBlockSMPlugin(ctx, *path2, *recType2, expr_projections_ptr2);
	...
	RawOperator* mml2 =  new MemMoveLocalTo(scan2, ctx, expr_projections_block_ptr2, 4);
	scan2->setParent(mml2);

	RawOperator* mmd2 =  new MemMoveDevice(mml2, ctx, expr_projections_block_ptr2, 4, false);
	mml2->setParent(mmd2);
	...
	expressions::Expression* e2 = new expressions::InputArgument(recType2, -1, atts2);
	expressions::Expression* e21 = new expressions::RecordProjection(e2, *d_key2);
	expressions::Expression* e4 = new expressions::InputArgument(recType2, -1, atts2);
	...
	expressions::Expression* pred2 = new expressions::LtExpression(new BoolType(), lhs2, rhs2);

	RawOperator* select2 = new Select(pred2, btt2);
	btt2->setParent(select2);

	RecordAttribute* join_key1 = new RecordAttribute(-1, "join198", key_str+"0", new IntType(), false);
	InputInfo * datasetInfo11 = catalog.getOrCreateInputInfo(join_key1->getRelationName());
	RecordType * rec1 = new RecordType{dynamic_cast<const RecordType &>(dynamic_cast<CollectionType *>(datasetInfo11->exprType)->getNestedType())};
	rec1->appendAttribute(join_key1);
	datasetInfo11->exprType = new BagType{*rec1};
	e31->registerAs(join_key1);

	RecordAttribute* join_key2 = new RecordAttribute(-1, "join198", key_str, new IntType(), false);
	InputInfo * datasetInfo21 = catalog.getOrCreateInputInfo(join_key2->getRelationName());
	RecordType * rec2 = new RecordType{dynamic_cast<const RecordType &>(dynamic_cast<CollectionType *>(datasetInfo21->exprType)->getNestedType())};
	...
	InputInfo * datasetInfoCoord = catalog.getOrCreateInputInfo(attr_target->getRelationName());
    ...
    HashPartitioner* hpart1 = new HashPartitioner(attr_target, build_e, build_widths, e31, select1, ctx, 15, "partition_hash_1");
    select1->setParent(hpart1);
    HashPartitioner* hpart2 = new HashPartitioner(attr_target, probe_e, probe_widths, e41, select2, ctx, 15, "partition_hash_2");
    select2->setParent(hpart2);

	RawOperator* join = new GpuPartitionedHashJoinChained(build_e, build_widths, e31, hpart1, probe_e, probe_widths, e41, hpart2, hpart1->getState(), hpart2->getState(), 15, ctx);
	...
	expressions::Expression* e5 = new expressions::InputArgument(recTypeJoin, -1, join_atts_d);
	expressions::Expression* e51 = new expressions::RecordProjection(e5, *join_key2);

	RecordAttribute* aggr = new RecordAttribute(-1, "agg199", "EXPR$0", new IntType(), false);
	InputInfo * datasetInfo31 = catalog.getOrCreateInputInfo(aggr->getRelationName());
	RecordType * rec3 = new RecordType{dynamic_cast<const RecordType &>(dynamic_cast<CollectionType *>(datasetInfo31->exprType)->getNestedType())};
	...
	RawOperator* reduce = new opt::GpuReduce(accs, outputExprs, new expressions::BoolConstant(true), join, ctx);
	join->setParent(reduce);

	vector<RecordAttribute*> reduce_attrs;
	reduce_attrs.push_back(aggr);
	RawOperator* gtc =  new GpuToCpu(reduce, ctx, reduce_attrs, 131072, gran_t::GRID);
	...
	expressions::Expression* e6 = new expressions::InputArgument(recTypeAggr, -1, aggr_atts_d);
...
void Query2 () {
	...
	RecordAttribute* d_key1 = new RecordAttribute(1, relName1, key_str, new IntType(), false); //recType1->getArg(key_str);
	RecordAttribute* d_payload1 = new RecordAttribute(2, relName1, payload_str1, new IntType(), false);
	std::cout << d_key1->getType() << std::endl;
	int attrNo = 1;
	const ExpressionType* recArgType = d_key1->getOriginalType();
	RecordAttribute* d_key_block1 = new RecordAttribute(1, relName1, key_str, recArgType, true);
	RecordAttribute* d_payload_block1 = new RecordAttribute(2, relName1, payload_str1, recArgType, true);
	std::cout << d_key_block1->getType() << std::endl;
	...
	expressions::Expression* e1 = new expressions::InputArgument(recType1, -1, atts1);
	expressions::Expression* e11 = new expressions::RecordProjection(e1, *d_key1);
	expressions::Expression* e12 = new expressions::RecordProjection(e1, *d_payload1);
	expressions::Expression* e3 = new expressions::InputArgument(recType1, -1, atts1);
	expressions::Expression* e31 = new expressions::RecordProjection(e3, *d_key1);
	expressions::Expression* e32 = new expressions::RecordProjection(e3, *d_payload1);
	...
	Exchange* in_xch11 = new Exchange (scan1, ctx, numPartitioners, expr_projections_blockd_ptr1, 4, NULL, true);
	scan1->setParent(in_xch11);

	//RawOperator* btt11 =  new BlockToTuples(scan1, ctx, exprs1, false, gran_t::THREAD);
	//scan1->setParent(btt11);

	RawOperator* btt11 =  new BlockToTuples(in_xch11, ctx, exprs1, false, gran_t::THREAD);
	in_xch11->setParent(btt11);

	RecordAttribute* hash_key1 = new RecordAttribute(-1, relName1, part_str + relName1, new IntType(), false);
	InputInfo * datasetInfoH1 = catalog.getOrCreateInputInfo(hash_key1->getRelationName());
	RecordType * hrec1 = new RecordType{dynamic_cast<const RecordType &>(dynamic_cast<CollectionType *>(datasetInfoH1->exprType)->getNestedType())};
	...
	expressions::Expression* h1 = new expressions::InputArgument(recTypeHash, -1, hash_atts_d);
	expressions::Expression* h11 = new expressions::RecordProjection(h1, *d_key1);
	expressions::Expression* h12 = new expressions::RecordProjection(h1, *d_payload1);

	vector<expressions::Expression*> hash_projections;
	hash_projections.push_back(h11);
	hash_projections.push_back(h12);

	RawOperator* part1 = new HashRearrange(btt11, ctx, numOfBuckets, hash_projections, h11, hash_key1);
	btt11->setParent(part1);

	expr_projections_blockd_ptr1.push_back(hash_key1);

	Exchange* in_xch12 = new Exchange (part1, ctx, 1, expr_projections_blockd_ptr1, 4, NULL, true, false, numPartitioners);
	...
	RecordAttribute* d_key2 = new RecordAttribute(1, relName2, key_str, new IntType(), false); //recType1->getArg(key_str);
	RecordAttribute* d_payload2 = new RecordAttribute(2, relName2, payload_str2, new IntType(), false); 
	std::cout << d_key2->getType() << std::endl;
	int attrNo2 = 1;
	const ExpressionType* recArgType2 = d_key2->getOriginalType();
	RecordAttribute* d_key_block2 = new RecordAttribute(1, relName2, key_str, recArgType2, true);
	RecordAttribute* d_payload_block2 = new RecordAttribute(2, relName2, payload_str2, recArgType2, true);
	...
	expressions::Expression* e2 = new expressions::InputArgument(recType2, -1, atts2);
	expressions::Expression* e21 = new expressions::RecordProjection(e2, *d_key2);
	expressions::Expression* e22 = new expressions::RecordProjection(e2, *d_payload2);
	expressions::Expression* e4 = new expressions::InputArgument(recType2, -1, atts2);
	expressions::Expression* e41 = new expressions::RecordProjection(e4, *d_key2);
	expressions::Expression* e42 = new expressions::RecordProjection(e4, *d_payload2);
	...
	//RawOperator* btt21 =  new BlockToTuples(scan2, ctx, exprs2, false, gran_t::THREAD);
	//scan2->setParent(btt21);

	Exchange* in_xch21 = new Exchange (scan2, ctx, numPartitioners, expr_projections_blockd_ptr2, 4, NULL, true);
	scan2->setParent(in_xch21);

	RawOperator* btt21 =  new BlockToTuples(in_xch21, ctx, exprs2, false, gran_t::THREAD);
	in_xch21->setParent(btt21);

	RecordAttribute* hash_key2 = new RecordAttribute(-1, relName2, part_str + relName2, new IntType(), false);
	InputInfo * datasetInfoH2 = catalog.getOrCreateInputInfo(hash_key2->getRelationName());
	RecordType * hrec2 = new RecordType{dynamic_cast<const RecordType &>(dynamic_cast<CollectionType *>(datasetInfoH2->exprType)->getNestedType())};
	...
	expressions::Expression* h2 = new expressions::InputArgument(recTypeHash2, -1, hash_atts_d2);
	expressions::Expression* h21 = new expressions::RecordProjection(h2, *d_key2);
	expressions::Expression* h22 = new expressions::RecordProjection(h2, *d_payload2);

	vector<expressions::Expression*> hash_projections2;
	hash_projections2.push_back(h21);
	hash_projections2.push_back(h22);

	RawOperator* part2 = new HashRearrange(btt21, ctx, numOfBuckets, hash_projections2, h21, hash_key2);
	btt21->setParent(part2);

	
	expr_projections_blockd_ptr2.push_back(hash_key2);

	Exchange* in_xch22 = new Exchange (part2, ctx, 1, expr_projections_blockd_ptr2, 4, NULL, true, false, numPartitioners);
	...
	expressions::Expression* f1 = new expressions::InputArgument(recTypeF, -1, f_atts_d);
	expressions::Expression* f11 = new expressions::RecordProjection(f1, *d_key_block1);
	expressions::Expression* f12 = new expressions::RecordProjection(f1, *d_payload_block1);
	expressions::Expression* f3 = new expressions::InputArgument(recTypeF, -1, f_atts_d);
	expressions::Expression* f31 = new expressions::RecordProjection(f3, *d_key_block1);
	expressions::Expression* f32 = new expressions::RecordProjection(f3, *d_payload_block1);

	RecordAttribute* f_key1 = new RecordAttribute(1, "coordinator_A", "f_key1", new IntType(), false);
	RecordAttribute* f_key_block1 = new RecordAttribute(*f_key1, true);
	RecordAttribute* f_payload1 = new RecordAttribute(2, "coordinator_A", "f_payload1", new IntType(), false);
	RecordAttribute* f_payload_block1 = new RecordAttribute(*f_payload1, true);

	InputInfo * datasetInfoC1 = catalog.getOrCreateInputInfo(f_key1->getRelationName());
	RecordType * crec1 = new RecordType{dynamic_cast<const RecordType &>(dynamic_cast<CollectionType *>(datasetInfoC1->exprType)->getNestedType())};
	...
	expressions::Expression* f2 = new expressions::InputArgument(recTypeF2, -1, f_atts_d2);
	expressions::Expression* f21 = new expressions::RecordProjection(f2, *d_key_block2);
	expressions::Expression* f22 = new expressions::RecordProjection(f2, *d_payload_block2);
	expressions::Expression* f4 = new expressions::InputArgument(recTypeF2, -1, f_atts_d2);
	expressions::Expression* f41 = new expressions::RecordProjection(f4, *d_key_block2);
	expressions::Expression* f42 = new expressions::RecordProjection(f4, *d_payload_block2);

	RecordAttribute* f_key2 = new RecordAttribute(-1, "coordinator_B", "f_key2", new IntType(), false);
	RecordAttribute* f_key_block2 = new RecordAttribute(*f_key2, true);
	RecordAttribute* f_payload2 = new RecordAttribute(-1, "coordinator_B", "f_payload2", new IntType(), false);
	RecordAttribute* f_payload_block2 = new RecordAttribute(*f_payload2, true);
	InputInfo * datasetInfoC2 = catalog.getOrCreateInputInfo(f_key2->getRelationName());
	RecordType * crec2 = new RecordType{dynamic_cast<const RecordType &>(dynamic_cast<CollectionType *>(datasetInfoC2->exprType)->getNestedType())};
	...
	expressions::Expression* fTarg = new expressions::InputArgument(recTypeTarget, -1, f_atts_target_d);
	expressions::Expression* dest_expr = new expressions::RecordProjection(fTarg, *attr_splitter);
	...
	/*ZipCollect* coord = new ZipCollect (attr_ptr, attr_splitter, attr_target, d_key_block1, d_key_block2,
										part1, part2, ctx, numOfBuckets, 
										hash_key1, expr_v1,
										hash_key2, expr_v2, "coordinator");*/

	ZipCollect* coord = new ZipCollect (attr_ptr, attr_splitter, attr_target, d_key_block1, d_key_block2,
										in_xch12, in_xch22, ctx, numOfBuckets, 
										hash_key1, expr_v1,
										hash_key2, expr_v2, "coordinator");
	...
	//Exchange* xch1 = new Exchange (coord, ctx, numConcurrent, f_atts_target_v, 4, expr_target, false);
	//coord->setParent(xch1);

	//std::cout << "exch1" << xch1 << std::endl;

	ZipInitiate* initiator = new ZipInitiate (attr_ptr, attr_splitter, attr_target, coord, ctx, numOfBuckets, coord->getStateLeft(), coord->getStateRight(), "launcher");
	coord->setParent(initiator);
	//ZipInitiate* initiator = new ZipInitiate (attr_ptr, attr_splitter, attr_target, xch1, ctx, numOfBuckets, coord->getStateLeft(), coord->getStateRight(), "launcher");
	//xch1->setParent(initiator);
	...
	//SplitOpt* splitop = new SplitOpt(initiator, ctx, 2, targets, 4, dest_expr, false, true);
	//initiator->setParent(splitop);

	//std::cout << "split " << splitop << std::endl;

	ZipForward* fwd1 = new ZipForward (attr_splitter, attr_target, d_key_block1, initiator, ctx, numOfBuckets, expr_v1, "forwarder", coord->getStateLeft());
	//splitop->setParent(fwd1);

	ZipForward* fwd2 = new ZipForward (attr_splitter, attr_target, d_key_block2, initiator, ctx, numOfBuckets, expr_v2, "forwarder", coord->getStateRight());
	...
	CollectionType *collTypeC1 = dynamic_cast<CollectionType*>(datasetInfoC1->exprType);
	...
	expressions::Expression* df1 = new expressions::InputArgument(new RecordType(recType_C1.getArgs()), -1, atts3);
	expressions::Expression* df11 = new expressions::RecordProjection(df1, *f_key1);
	expressions::Expression* df12 = new expressions::RecordProjection(df1, *f_payload1);
	expressions::Expression* df3 = new expressions::InputArgument(new RecordType(recType_C1.getArgs()), -1, atts3);
	expressions::Expression* df31 = new expressions::RecordProjection(df3, *f_key1);
	expressions::Expression* df32 = new expressions::RecordProjection(df3, *f_payload1);

	vector<expressions::Expression*> exprs3;
	exprs3.push_back(df11);
	exprs3.push_back(df12);

	RawOperator* mml1 =  new MemMoveLocalTo(fwd1, ctx, expr_projections_block_ptr1, 4);
	fwd1->setParent(mml1);
	...
	CollectionType *collTypeC2 = dynamic_cast<CollectionType*>(datasetInfoC2->exprType);
	...
	expressions::Expression* df2 = new expressions::InputArgument(new RecordType(recType_C2.getArgs()), -1, atts4);
	expressions::Expression* df21 = new expressions::RecordProjection(df2, *f_key2);
	expressions::Expression* df22 = new expressions::RecordProjection(df2, *f_payload2);
	expressions::Expression* df4 = new expressions::InputArgument(new RecordType(recType_C2.getArgs()), -1, atts4);
	expressions::Expression* df41 = new expressions::RecordProjection(df4, *f_key2);
	expressions::Expression* df42 = new expressions::RecordProjection(df4, *f_payload2);

	vector<expressions::Expression*> exprs4;
	exprs4.push_back(df21);
	exprs4.push_back(df22);

	RawOperator* mml2 =  new MemMoveLocalTo(fwd2, ctx, expr_projections_block_ptr2, 4);
	fwd2->setParent(mml2);
	...
	expressions::Expression* dummy = new expressions::InputArgument(recTypeTarget2, -1, f_atts_dummy_d);
	expressions::Expression* dummy1 = new expressions::RecordProjection(dummy, *attr_target);

	vector<expressions::Expression*> expr_dummy;
	expr_dummy.push_back(dummy1);

	RecordAttribute* join_key1 = new RecordAttribute(1, "join198", key_str+"0", new IntType(), false);
	RecordAttribute* join_payload1 = new RecordAttribute(2, "join198", payload_str1, new IntType(), false);
	InputInfo * datasetInfo11 = catalog.getOrCreateInputInfo(join_key1->getRelationName());
	RecordType * rec1 = new RecordType{dynamic_cast<const RecordType &>(dynamic_cast<CollectionType *>(datasetInfo11->exprType)->getNestedType())};
	rec1->appendAttribute(join_key1);
	...
	RecordAttribute* join_key2 = new RecordAttribute(1, "join198", key_str, new IntType(), false);
	RecordAttribute* join_payload2 = new RecordAttribute(2, "join198", payload_str2, new IntType(), false);
	InputInfo * datasetInfo21 = catalog.getOrCreateInputInfo(join_key2->getRelationName());
	RecordType * rec2 = new RecordType{dynamic_cast<const RecordType &>(dynamic_cast<CollectionType *>(datasetInfo21->exprType)->getNestedType())};
	rec2->appendAttribute(join_key2);
    ...
    HashPartitioner* hpart1 = new HashPartitioner(attr_target, build_e, build_widths, df31, select1, ctx, 13, "partition_hash_1");
    select1->setParent(hpart1);

    expressions::Expression* lhs3 = dummy1;
	expressions::Expression* rhs3 = new expressions::IntConstant(-1);
	expressions::Expression* pred3 = new expressions::LtExpression(new BoolType(), lhs3, rhs3);

	//RawOperator* select3 = new Select(pred3, hpart1);
	//hpart1->setParent(select3);
    //GpuHashRearrange* ttb1 = new GpuHashRearrange(select3, ctx, 1, expr_dummy, dummy1);
    //select3->setParent(ttb1);

    //GpuToCpu* gtc1 = new GpuToCpu(ttb1, ctx, unionFields, 131072);
    //ttb1->setParent(gtc1);

    HashPartitioner* hpart2 = new HashPartitioner(attr_target, probe_e, probe_widths, df41, select2, ctx, 13, "partition_hash_2");
    select2->setParent(hpart2);

    expressions::Expression* lhs4 = dummy1;
	expressions::Expression* rhs4 = new expressions::IntConstant(-1);
	expressions::Expression* pred4 = new expressions::LtExpression(new BoolType(), lhs4, rhs4);

	//RawOperator* select4 = new Select(pred4, hpart2);
	//hpart2->setParent(select4);
    //GpuHashRearrange* ttb2 = new GpuHashRearrange(select4, ctx, 1, expr_dummy, dummy1);
    //select4->setParent(ttb2);
    ...
    //RawOperator* btt =  new BlockToTuples(ctg, ctx, expr_dummy, true, gran_t::GRID);
	//ctg->setParent(btt);

	RawOperator* join = new GpuPartitionedHashJoinChained(build_e, build_widths, e31, hpart1, probe_e, probe_widths, e41, hpart2, hpart1->getState(), hpart2->getState(), 13, ctx, "hj_part", pip_rcv, NULL);
	...
	RecordType *recTypeJoin = new RecordType(join_atts);
	expressions::Expression* e5 = new expressions::InputArgument(recTypeJoin, -1, join_atts_d);
	expressions::Expression* e51 = new expressions::RecordProjection(e5, *join_payload1);
	expressions::Expression* e52 = new expressions::RecordProjection(e5, *join_payload2);

	RecordAttribute* aggr1 = new RecordAttribute(1, "agg199", "EXPR$0", new IntType(), false);
	RecordAttribute* aggr2 = new RecordAttribute(2, "agg199", "EXPR$1", new IntType(), false);
	InputInfo * datasetInfo31 = catalog.getOrCreateInputInfo(aggr1->getRelationName());
	RecordType * rec3 = new RecordType{dynamic_cast<const RecordType &>(dynamic_cast<CollectionType *>(datasetInfo31->exprType)->getNestedType())};
	...
	//Exchange* xch2 = new Exchange (gtc, ctx, 1, reduce_attrs, 4, new expressions::IntConstant(0), false, false, numConcurrent);
	//gtc->setParent(xch2);


	RecordAttribute* aggr3 = new RecordAttribute(1, "agg200", "EXPR$0", new IntType(), false);
	RecordAttribute* aggr4 = new RecordAttribute(2, "agg200", "EXPR$1", new IntType(), false);
	InputInfo * datasetInfo41 = catalog.getOrCreateInputInfo(aggr3->getRelationName());
	RecordType * rec4 = new RecordType{dynamic_cast<const RecordType &>(dynamic_cast<CollectionType *>(datasetInfo41->exprType)->getNestedType())};
	...
	//RawOperator* reduce2 = new opt::Reduce(accs, printExprs, new expressions::BoolConstant(true), xch2, ctx);
	//xch2->setParent(reduce2);
	RawOperator* reduce2 = new opt::Reduce(accs, printExprs, new expressions::BoolConstant(true), gtc, ctx);
	...
	expressions::Expression* e7 = new expressions::InputArgument(recTypeAggr2, -1, aggr_atts_d2);
	...
	/*RecordAttribute* aggr2 = new RecordAttribute(-1, "agg199", "EXPR$0", new IntType(), false);
	InputInfo * datasetInfo101 = catalog.getOrCreateInputInfo(aggr2->getRelationName());
	RecordType * rec101 = new RecordType{dynamic_cast<const RecordType &>(dynamic_cast<CollectionType *>(datasetInfo101->exprType)->getNestedType())};
	...
	RawOperator* red = new opt::Reduce(accs, outputExprs2, new expressions::BoolConstant(true), btt11, ctx, false, "/home/sioulas/pelago/pelago/res.json");
	btt11->setParent(red);*/
...
/*void Query3 () {
	...
	//RawOperator* mml1 =  new MemMoveLocalTo(ch_left, ctx, expr_projections_block_ptr1, 4);
	//ch_left->setParent(mml1);

	RawOperator* btt12 =  new BlockToTuples(ch_left, ctx, exprs3, false, gran_t::THREAD);
	ch_left->setParent(btt12);
	...
	//RawOperator* mml2 =  new MemMoveLocalTo(ch_right, ctx, expr_projections_block_ptr2, 4);
	//ch_right->setParent(mml2);

	RawOperator* btt22 =  new BlockToTuples(ch_right, ctx, exprs4, false, gran_t::THREAD);
	ch_right->setParent(btt22);
	...
	RawOperator* join = new HashJoinChained(build_e, build_widths, df31, select1, probe_e, probe_widths, df41, select2, 28, ctx, 128000000);
	...
	RawOperator* reduce = new opt::Reduce(accs, outputExprs, new expressions::BoolConstant(true), join, ctx, false, "/home/sioulas/pelago/pelago/res.json");
	join->setParent(reduce);
