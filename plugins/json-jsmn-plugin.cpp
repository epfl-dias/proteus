...
RawValueMemory JSONPlugin::initCollectionUnnest(RawValue val_parentTokenNo)
{
	...
	AllocaInst* mem_currentToken = context->CreateEntryBlockAlloca(F,
			string("currentTokenUnnested"), int64Type);
	Value* val_1 = Builder->getInt64(1);
	Value* val_currentToken = Builder->CreateAdd(val_parentTokenNo.value,
			val_1);
...
RawValueMemory JSONPlugin::collectionGetNext(RawValueMemory mem_currentToken)
{
	...
	Value* token_i_contents_start = context->getStructElem(
...
void JSONPlugin::scanObjects(const RawOperator& producer, Function* debug)
{
	...
	context->CreateForLoop("jsonScanCond", "jsonScanBody", "jsonScanInc",
			"jsonScanEnd", &jsonScanCond, &jsonScanBody, &jsonScanInc,
			&jsonScanEnd);
		...
		Bindings wrappedBindings, const char* path)
{
	...
	Value* token_parent_end = context->getStructElem(mem_tokens_parent_shifted,
			2);
...
//	case STRING:	cout << "Dealing with (and initializing) String" << endl;
	case RECORD:
	case LIST: {
...
//	BasicBlock *ifBlock, *elseBlock, *endBlock;
//	endBlock = BasicBlock::Create(llvmContext, "afterReadValueHash", F);
//	context->CreateIfElseBlocks(context->getGlobalFunction(), "ifPathHash", "elsePathNullEqHash",
...
int JSONPlugin::readPathInterpreted(int parentToken, list<string> path)
{
	...
	for (int i = parentToken + 1; tokens[i].end <= tokens[parentToken].end; i +=
			2)
	{
		if (TOKEN_STRING(buf,tokens[i],key.c_str()))
		{
...
//		}
		...
		while (tokens[i].start < tokens[tokenNo].end && tokens[i].start != 0)
		{
			string attrName = string(buf + tokens[i].start,
	...
	for (int i = 1; tokens[i].start != 0;)
	{
		...
		if (tokens[neededToken].type != 2)
		{
			string error_msg = string(
					"[JSON Plugin - jsmn: ]: Can only unnest collections!");
			LOG(ERROR)<< error_msg;
			throw runtime_error(string(error_msg));
		}
		else
