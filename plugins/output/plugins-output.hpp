...
public:
	Materializer(const vector<RecordAttribute*>& whichFields, const vector<materialization_mode>& outputMode_);
	~Materializer()	{}

	const vector<RecordAttribute*>& getWantedFields() const			{ return wantedFields; }
	const vector<materialization_mode>& getOutputMode() const		{ return outputMode; }
...
class OutputPlugin {
public:
	OutputPlugin(RawContext* const context, const Materializer& materializer_, const std::map<string, AllocaInst*>& bindings);
	~OutputPlugin()	{}

	llvm::StructType* getPayloadType()										{ return payloadType; }
	int getPayloadTypeSize()												{ return payloadTypeSize; }
	const std::map<string, AllocaInst*>& getBindings() const				{ return currentBindings; }
	std::vector<Type*>* getMaterializedTypes()								{ return materializedTypes; }
...
private:
	...
	const std::map<string, AllocaInst*>& currentBindings;
	llvm::StructType* payloadType;
	std::vector<Type*>* materializedTypes;
	int payloadTypeSize;

	Type* chooseType(ExpressionType* exprType, Type* currType, materialization_mode mode);
};
