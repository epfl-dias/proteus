...
constexpr auto query = "ssb1000_Q4_1";
...
PreparedStatement Query::prepare41(bool memmv) {
  ...
  auto rel51018 =
      getBuilder<Tplugin>()
          .scan(
              "inputs/ssbm1000/part.csv", {"p_partkey", "p_mfgr"}, getCatalog(),
              pg{Tplugin::
  ...
  rel =
      rel.to_gpu()   // (trait=[Pelago.[].packed.NVPTX.homRandom.hetSingle])
          ...
          //          .project([&](const auto &arg) -> std::vector<expression_t>
          //          {
          //            return {(arg["lo_custkey"]).as("eager", "lo_custkey"),
          //            (arg["lo_partkey"]).as("eager",
          //                                                                                          "lo_partkey"), (arg["lo_suppkey"]).as("eager", "lo_suppkey"), (arg["lo_orderdate"]).as("eager",
          //                                                                                                                                                                                 "lo_orderdate"), (arg["lo_revenue"]).as("eager", "lo_revenue"), (arg["lo_supplycost"]).as("eager", "lo_supplycost")};
          //          })  // (s_suppkey=[$0],
          .join(
              rel51028,
              ...
              20,
              512 * 1024)  // (condition=[=($3, $0)], joinType=[inner],
          ...
          .groupby(
              ...
              7,
              64 * 1024)  // (group=[{0, 1}], profit=[SUM($2)],
