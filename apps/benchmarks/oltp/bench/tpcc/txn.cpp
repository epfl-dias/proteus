...
bool TPCC::exec_txn(txn::TransactionExecutor &executor, txn::Txn &txn,
  ...
  switch (q->query_type) {
    case NEW_ORDER:
      return exec_neworder_txn(q, txn);
    case PAYMENT:
      return exec_payment_txn(q, txn);
    case ORDER_STATUS:
      return exec_orderstatus_txn(q, txn);
    case DELIVERY:
      return exec_delivery_txn(q, txn);
    case STOCK_LEVEL:
      return exec_stocklevel_txn(q, txn);
    default:
      assert(false);
      break;
  }
...
bool TPCC::exec_micro_scan_stock(txn::Txn &txn) {
  //  time_block t("exec_micro_scan_stock");
  constexpr uint n_scan_columns = 1;

  constexpr column_id_t stock_col_read[] = {
  ...
  size_t num_stock = TPCC_MAX_ITEMS * (this->num_warehouse);
  struct st_read st_rec_accumulator = {};

  for (auto i = 0; i < num_stock; i++) {
    ...
    record_ptr->readWithLatch([&](global_conf::IndexVal *idx_ptr) {
      table_stock->getIndexedRecord(txn.txnTs, *idx_ptr, &st_rec,
                                    stock_col_read, n_scan_columns);
    });

    st_rec_accumulator.s_quantity += st_rec.s_quantity;
    st_rec_accumulator.s_ytd += st_rec.s_ytd;
    st_rec_accumulator.s_order_cnt += st_rec.s_order_cnt;
    st_rec_accumulator.s_remote_cnt += st_rec.s_remote_cnt;
  }
  return true;
}
...
bool TPCC::exec_neworder_txn(const struct tpcc_query *q, txn::Txn &txn) {
  ...
  struct st_read st_rec = {};
  constexpr column_id_t stock_col_rw[] = {
      2, TPCC_NDIST_PER_WH + 3, TPCC_NDIST_PER_WH + 4, TPCC_NDIST_PER_WH + 5};
  ...
  w_idx_ptr->readWithLatch([&](global_conf::IndexVal *idx_ptr) {
    constexpr column_id_t w_col_scan[] = {7};  // position in columns
  ...
  constexpr column_id_t cust_col_scan[] = {15};
...
#if !tpcc_dist_txns
  assert(storage::StorageUtils::get_pid(c_idx_ptr->VID) == txn.partition_id);
#endif
  ...
  void *o_idx_ptr = table_order->insertRecord(
...
bool TPCC::exec_orderstatus_txn(const struct tpcc_query *stmts, txn::Txn &txn) {
  // int ol_number = 1;

  assert(index_on_order_tbl &&
         "orderstatus txn requires index on orderline table");

  uint32_t cust_id = std::numeric_limits<uint32_t>::max();
  if (stmts->by_last_name) {
    /*
    ...
    if (!this->cust_sec_index->find(
            cust_derive_key(stmts->c_last, stmts->c_d_id, stmts->c_w_id), sr)) {
    ...
    uint nmatch = fetch_cust_records(sr, c_recs, txn.txnTs);
    ...
    cust_id = MAKE_CUST_KEY(stmts->c_w_id, stmts->c_d_id, stmts->c_id);
    ...
    c_idx_ptr->readWithLatch([&](global_conf::IndexVal *idx_ptr) {
      constexpr column_id_t cust_col_scan_read[] = {3, 4, 5, 15};
      table_customer->getIndexedRecord(txn.txnTs, *idx_ptr, &c_r,
                                       cust_col_scan_read, 4);
    });
    ...
    for (int o = TPCC_NCUST_PER_DIST - 1, i = 0; o > 0; o--, i++) {
      ...
      o_idx_ptr->readWithLatch([&](global_conf::IndexVal *idx_ptr) {
        constexpr column_id_t o_col_scan[] = {0, 4, 5, 6};
        table_order->getIndexedRecord(txn.txnTs, *idx_ptr, &p_or[i], o_col_scan,
                                      4);
      });
      ...
      for (auto ol_number = 0; ol_number < p_or[i].o_ol_cnt; ++ol_number) {
        ...
        ol_idx_ptr->readWithLatch([&](global_conf::IndexVal *idx_ptr) {
          constexpr column_id_t ol_col_scan[] = {0, 5, 6, 7, 8};
...
bool TPCC::exec_stocklevel_txn(const struct tpcc_query *q, txn::Txn &txn) {
  ...
  */
  ...
  d_idx_ptr->readWithLatch([&](global_conf::IndexVal *idx_ptr) {
    constexpr column_id_t dist_col_scan_read[] = {10};
    table_district->getIndexedRecord(txn.txnTs, *idx_ptr, &o_id,
  ...
  while (ol_o_id < o_id) {
    ...
    o_idx_ptr->readWithLatch([&](global_conf::IndexVal *idx_ptr) {
      constexpr column_id_t o_col_scan_read[] = {6};
      table_order->getIndexedRecord(txn.txnTs, *idx_ptr, &rec_ol_count,
                                    o_col_scan_read, 1);
    });
    //---
    ...
    while (ol_number < rec_ol_count) {
      ...
      ol_idx_ptr->readWithLatch([&](global_conf::IndexVal *idx_ptr) {
        constexpr column_id_t ol_col_scan_read[] = {4};
        table_order_line->getIndexedRecord(txn.txnTs, *idx_ptr, &ol_i_id,
                                           ol_col_scan_read, 1);
      });
      ...
      st_idx_ptr->readWithLatch([&](global_conf::IndexVal *idx_ptr) {
        constexpr column_id_t st_col_scan_read[] = {2};
        table_stock->getIndexedRecord(txn.txnTs, *idx_ptr, &s_quantity,
                                      st_col_scan_read, 1);
      });
...
inline uint TPCC::fetch_cust_records(const struct secondary_record &sr,
                                     struct cust_read *c_recs,
                                     const txn::TxnTs &txnTs) {
  uint nmatch = 0;
  ...
  for (int i = 0; i < sr.sr_nids; i++) {
    ...
    c_idx_ptr->readWithLatch([&](global_conf::IndexVal *idx_ptr) {
      table_customer->getIndexedRecord(txnTs, *idx_ptr, &c_recs[nmatch],
                                       c_col_scan, 5);
    });
...
bool TPCC::exec_payment_txn(const struct tpcc_query *stmts, txn::Txn &txn) {
  // // Updates..
  ...
  // update warehouse and then release then write lock

  double w_ytd = 0.0;
  const column_id_t wh_col_scan_upd[] = {7};

  w_idx_ptr->writeWithLatch(
      [&](global_conf::IndexVal *idx_ptr) {
        table_warehouse->getIndexedRecord(txn.txnTs, *idx_ptr, &w_ytd,
                                          wh_col_scan_upd, 1);

        w_ytd += stmts->h_amount;
        table_warehouse->updateRecord(txn, idx_ptr, &w_ytd, wh_col_scan_upd, 1);
        idx_ptr->write_lck.unlock();
      },
      txn, table_warehouse->table_id);
  ...
  d_idx_ptr->writeWithLatch(
      [&](global_conf::IndexVal *idx_ptr) {
        table_district->getIndexedRecord(txn.txnTs, *idx_ptr, &d_ytd,
                                         d_col_scan_upd, 1);
        d_ytd += stmts->h_amount;
        table_district->updateRecord(txn, idx_ptr, &d_ytd, d_col_scan_upd, 1);
        idx_ptr->write_lck.unlock();
      },
      txn, table_district->table_id);
  ...
  c_idx_ptr->writeWithLatch(
      [&](global_conf::IndexVal *idx_ptr) {
        table_customer->getIndexedRecord(txn.txnTs, *idx_ptr, &cust_rw,
                                         cust_col_scan_read, 5);
        ...
        if (cust_rw.c_credit[0] == 'B' && cust_rw.c_credit[1] == 'C') {
          ...
          /*=====================================================+
            EXEC SQL SELECT c_data
                  ...
                  +=====================================================*/
          // char c_new_data[501];
          sprintf(cust_rw.c_data, "| %4d %2d %4d %2d %4d $%7.2f", stmts->c_id,
                  stmts->c_d_id, stmts->c_w_id, stmts->d_id, stmts->w_id,
                  stmts->h_amount);
          strncat(cust_rw.c_data, cust_rw.c_data, 500 - strlen(cust_rw.c_data));
        }

        table_customer->updateRecord(txn, idx_ptr, &cust_rw, cust_col_scan_read,
                                     num_col_upd);

        c_idx_ptr->write_lck.unlock();
      },
      txn, table_customer->table_id);
  //---
  ...
  */
  ...
  /*=============================================================================+
    ...
    :h_amount, :h_data);
    +=============================================================================*/
#if payment_txn_insert_history

  struct tpcc_history h_ins {};
  h_ins.h_c_id = stmts->c_id;
  h_ins.h_c_d_id = stmts->c_d_id;
  h_ins.h_c_w_id = stmts->c_w_id;
  h_ins.h_d_id = stmts->d_id;
  h_ins.h_w_id = stmts->w_id;
  h_ins.h_date = get_timestamp();
  h_ins.h_amount = stmts->h_amount;

  void *hist_idx_ptr = table_history->insertRecord(
      &h_ins, txn.txnTs.txn_start_time, txn.partition_id, txn.master_version);
#endif
