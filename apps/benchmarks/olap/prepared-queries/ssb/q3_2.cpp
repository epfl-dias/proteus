...
PreparedStatement ssb::Query::prepare32(proteus::QueryShaper &morph) {
  ...
  auto rel34584 = morph.scan("date", {"d_datekey", "d_year"});

  auto rel34589 = morph.scan("customer", {"c_custkey", "c_city", "c_nation"});

  auto rel34594 = morph.scan("supplier", {"s_suppkey", "s_city", "s_nation"});
  ...
  return morph
      .parallel(
          rel, {rel34584, rel34589, rel34594},
          [](RelBuilder probe, std::vector<RelBuilder> build) {
            auto rel34584_d = build.at(0).unpack().filter(
