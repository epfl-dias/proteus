...
void tpchGroup(map<string, dataset> datasetCatalog, int predicate,
               int aggregatesNo);
void tpchMat(map<string, dataset> datasetCatalog, int predicate,
             int aggregatesNo);
...
int main() {
  ...
  for (int i = 0; i < runs; i++) {
    cout << "[tpch-json-groups-cached: ] Run " << i + 1 << endl;
  ...
  rawCatalog.clear();
  cache.clear();
  cout << "Query 0 (Mat. - Cache creation)" << endl;
  tpchMat(datasetCatalog, predicateMax, 2);
  for (int i = 0; i < runs; i++) {
  ...
  cache.clear();
  cout << "Query 0 (Mat. - Cache creation)" << endl;
  tpchMat(datasetCatalog, predicateMax, 3);
  for (int i = 0; i < runs; i++) {
    cout << "[tpch-bin-joins: ] Run " << i + 1 << endl;
    for (int i = 1; i <= selectivityShifts; i++) {
      double ratio = (i / (double)10);
  ...
  cache.clear();
  cout << "Query 0 (Mat. - Cache creation)" << endl;
  tpchMat(datasetCatalog, predicateMax, 4);
  for (int i = 0; i < runs; i++) {
    cout << "[tpch-bin-joins: ] Run " << i + 1 << endl;
    for (int i = 1; i <= selectivityShifts; i++) {
      double ratio = (i / (double)10);
...
void tpchGroup(map<string, dataset> datasetCatalog, int predicate,
  ...
  if (aggregatesNo == 2) {
    ...
    expressions::Expression *outputExpr2 = new expressions::RecordProjection(
        quantity->getOriginalType(), nestArg, *quantity);
  ...
  if (aggregatesNo == 3) {
    ...
    expressions::Expression *outputExpr3 = new expressions::RecordProjection(
        extendedprice->getOriginalType(), nestArg, *extendedprice);
  ...
  if (aggregatesNo == 4) {
    ...
    expressions::Expression *outputExpr4 = new expressions::RecordProjection(
        tax->getOriginalType(), nestArg, *tax);
...
void tpchMat(map<string, dataset> datasetCatalog, int predicate,
  ...
  if (aggregatesNo == 2) {
    ...
    ExprMaterializer *mat1 =
        new ExprMaterializer(toMat1, lineHint, mat0, &ctx, matLabel1);
    mat0->setParent(mat1);
    lastMat = mat1;
  }
  if (aggregatesNo == 3) {
  ...
  if (aggregatesNo == 4) {
    ...
    expressions::Expression *toMat3 =
        new expressions::RecordProjection(tax->getOriginalType(), arg, *tax);
  ...
  radix::Nest *nestOp =
      new radix::Nest(&ctx, accs, outputExprs, aggrLabels, predNest, f, f,
                      lastMat, nestLabel, *mat);
  lastMat->setParent(nestOp);
