...
PreparedStatement Q<19>::cpar(DegreeOfParallelism dop, Tp aff_parallel,
  ...
  auto rel1618 =
      getBuilder<Tplugin>()
          .template scan<Tplugin>(
              ...
              getCatalog())  // (table=[[SSB, ch100w_item]], fields=[[0, 3]],
  ...
  return getBuilder<Tplugin>()
      .template scan<Tplugin>(
          "tpcc_orderline", {"ol_w_id", "ol_i_id", "ol_quantity", "ol_amount"},
          getCatalog())  // (table=[[SSB, ch100w_orderline]], fields=[[2, 4, 7,
                         // 8]],
                         // traits=[Pelago.[].X86_64.packed.homSingle.hetSingle.none])
      .router(
      ...
      .join(
          ...
          27,
          100000)  // (condition=[=($3, $0)], joinType=[inner], rowcnt=[2.56E7],
                   // maxrow=[100000.0], maxEst=[100000.0], h_bits=[27],
                   // build=[RecordType(INTEGER i_id, BOOLEAN >=, BOOLEAN <=)],
                   // lcount=[1.3944357272154548E9], rcount=[3.84E9],
                   // buildcountrow=[2.56E7], probecountrow=[3.84E9])
      .project([&](const auto &arg) -> std::vector<expression_t> {
      ...
      .print(
          ...
          std::string{query} +
              (memmv
                   ? "mv"
                   : "nmv"))  // (trait=[ENUMERABLE.[].X86_64.unpckd.homSingle.hetSingle.cX86_64])
      .prepare();
}
