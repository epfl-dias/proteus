...
void symantecCSV1Caching(map<string, dataset> datasetCatalog) {
  ...
  ExprMaterializer *mat3 =
      new ExprMaterializer(toMat3, linehint, mat2, &ctx, matLabel3);
  mat2->setParent(mat3);

  /* XXX Not strictly needed for this query */
  expressions::Expression *toMat4 =
      new expressions::RecordProjection(size->getOriginalType(), arg, *size);
  char matLabel4[] = "sizeMaterializer";
  ExprMaterializer *mat4 =
      new ExprMaterializer(toMat4, linehint, mat3, &ctx, matLabel4);
  mat3->setParent(mat4);
  ...
  opt::Reduce *reduce =
      new opt::Reduce(accs, outputExprs, predicate, lastMat, &ctx);
...
void symantecCSV1CachingB(map<string, dataset> datasetCatalog) {
  ...
  //    char matLabel4[] = "sizeMaterializer";
  //    ExprMaterializer *mat4 = new ExprMaterializer(toMat4, linehint, mat3,
  //    &ctx,
  //            matLabel4);
  //    mat3->setParent(mat4);
...
void symantecCSV2Caching(map<string, dataset> datasetCatalog) {
  ...
  /* Pred: */
  expressions::Expression *selClass = new expressions::RecordProjection(
      classa->getOriginalType(), arg, *classa);
  expressions::Expression *predExpr1 = new expressions::IntConstant(classNo);
  expressions::Expression *predicate =
      new expressions::EqExpression(selClass, predExpr1);

  opt::Reduce *reduce =
      new opt::Reduce(accs, outputExprs, predicate, mat, &ctx);
...
void symantecCSV4v2(map<string, dataset> datasetCatalog) {
  ...
  //    expressions::Expression* selID      =
  //                new
  //                expressions::RecordProjection(id->getOriginalType(),arg,*id);
  ...
  //    expressions::IntConstant(idLow); expressions::Expression* predExpr2 =
  //    new expressions::IntConstant(idHigh);
  ...
  expressions::Expression *predicateNum1 =
      new expressions::GtExpression(selSize, predExpr1);
...
void symantecCSV5(map<string, dataset> datasetCatalog) {
  ...
  /**
   * SELECT
   * id > 90000000 and id < 130000000 and bot = 'FESTI' and size < 1500 and
  ...
  expressions::Expression *selBot =
      new expressions::RecordProjection(bot->getOriginalType(), arg, *bot);
  expressions::Expression *selCode = new expressions::RecordProjection(
      country_code->getOriginalType(), arg, *country_code);

  expressions::Expression *predExpr1 = new expressions::IntConstant(idLow);
  expressions::Expression *predExpr2 = new expressions::IntConstant(idHigh);
  expressions::Expression *predExpr3 = new expressions::IntConstant(sizeHigh);
  expressions::Expression *predExpr4 = new expressions::StringConstant(botName);
  expressions::Expression *predExpr5a =
      new expressions::StringConstant(countryCode1);
  expressions::Expression *predExpr5b =
      new expressions::StringConstant(countryCode2);
  expressions::Expression *predExpr5c =
      new expressions::StringConstant(countryCode3);
  ...
  // size < 1500
  expressions::Expression *predicateNum2 =
      new expressions::LtExpression(selSize, predExpr3);
  ...
  //(country_code = 'US' OR country_code = 'RU' OR country_code = 'IN')
  expressions::Expression *predicateCodeStrA =
      new expressions::EqExpression(selCode, predExpr5a);
  expressions::Expression *predicateCodeStrB =
      new expressions::EqExpression(selCode, predExpr5b);
  expressions::Expression *predicateCodeStrC =
      new expressions::EqExpression(selCode, predExpr5c);

  expressions::Expression *predicateOr_ =
      new expressions::OrExpression(predicateCodeStrA, predicateCodeStrB);
  expressions::Expression *predicateStr2 =
      new expressions::OrExpression(predicateOr_, predicateCodeStrC);
...
void symantecCSV6(map<string, dataset> datasetCatalog) {
  ...
  expressions::Expression *predicate3 =
      new expressions::LtExpression(selClass, predExpr3);
...
// where id > 90000000 and id < 130000000 and size < 1500 and classa > 50 and
// classa < 60 group by classa;
/*
 * Since caches are columnar, intuitively so should the operators
 * for better locality
 */
void symantecCSV7(map<string, dataset> datasetCatalog) {
  ...
  /**
   * SELECT
   * id > 90000000 and id < 130000000 and size < 1500 and classa > 50 and classa
   * < 60
   */
  list<RecordAttribute> argSelections;
  argSelections.push_back(*id);
  ...
  expressions::Expression *predicate4 =
      new expressions::GtExpression(selClass, predExpr4);
  expressions::Expression *predicate5 =
      new expressions::LtExpression(selClass, predExpr5);
  expressions::Expression *predicateAnd2 =
      new expressions::AndExpression(predicate4, predicate5);
