...
void tpchOrderSelection1CachingPred(map<string,dataset> datasetCatalog, vector<int> predicates);
...
void tpchOrderSelection1CachingPred(map<string, dataset> datasetCatalog,
	...
	/* Output: */
	expressions::Expression* outputExpr = new expressions::IntConstant(1);
	ReduceNoPred *reduce = new ReduceNoPred(SUM, outputExpr, lastSelectOp,
			&ctx);
	lastSelectOp->setParent(reduce);
