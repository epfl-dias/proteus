...
dbCreateTable(conn = con, name = "test12", fields = list(a = "integer", b = "varchar"), path = "mock.csv", linehint = 20000, type = "csv")
test <- tbl(con, "test12")
test %>% filter(a > 0.0) %>% summarise(colb = b)
...
iris_tbl <- tbl(con, "iris3")
iris_tbl %>% summarise(Sepal.Length)
