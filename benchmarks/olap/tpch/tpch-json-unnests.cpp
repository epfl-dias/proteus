...
int main() {
  ...
  /* Schema + Caching */
  cout << "[tpch-json-unnests: ] Caching predicate while exploiting schema"
       << endl;
  tpchUnnestCachingPred(datasetCatalog, predicateMax, exploitSchema);
  cout << "[tpch-json-unnests: ] End of Caching predicate while exploiting "
          "schema"
       << endl;
...
void tpchUnnestCachingPred(map<string, dataset> datasetCatalog, int predicate,
  ...
  jsonPipelined::JSONPlugin *pg = new jsonPipelined::JSONPlugin(
      &ctx, fileName, ordersDocType, linehint, exploitSchema);
