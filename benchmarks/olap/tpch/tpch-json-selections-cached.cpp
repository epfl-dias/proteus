...
void tpchOrderSelection1(map<string, dataset> datasetCatalog,
                         vector<int> predicates, bool exploitSchema);
...
int main() {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    cout << "FILE-CONSCIOUS ReadPath() + CACHING (MATERIALIZING): Pred" << endl;
    for (int i = 1; i <= 10; i++) {
      double ratio = (i / (double)10);
      double percentage = ratio * 100;
      int predicateVal = (int)ceil(pred1 * ratio);
...
//
//    for (int i = 0; i < 5; i++) {
//        cout << "[tpch-json-selections-cached: ] Baseline Run " << i + 1 <<
//        endl; cout << "Building PM" << endl; vector<int> predicates;
//        predicates.push_back(pred1);
//        tpchOrderSelection1(datasetCatalog, predicates, false);
//
...
//            << endl;
//    for (int i = 0; i < 5; i++) {
//        cout << "[tpch-json-selections-cached: Schema+Pred] Run " << i + 1 <<
//        endl; for (int i = 1; i <= 10; i++) {
//            double ratio = (i / (double) 10);
//            double percentage = ratio * 100;
//            int predicateVal = (int) ceil(pred1 * ratio);
//            cout << "SELECTIVITY FOR key < " << predicateVal << ": "
...
void tpchOrderSelection1(map<string, dataset> datasetCatalog,
  ...
  jsonPipelined::JSONPlugin *pg = new jsonPipelined::JSONPlugin(
      &ctx, fname, documentType, linehint, exploitSchema);
