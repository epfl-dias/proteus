...
void tpchJoin1a(map<string, dataset> datasetCatalog, int predicate) {
  ...
  jsonPipelined::JSONPlugin *pgOrders = new jsonPipelined::JSONPlugin(
      &ctx, ordersName, ordersDocType, ordersLinehint);
  ...
  jsonPipelined::JSONPlugin *pgLineitem = new jsonPipelined::JSONPlugin(
      &ctx, lineitemName, lineitemDocType, lineitemLinehint);
  ...
  RecordAttribute *projTupleL =
      new RecordAttribute(ordersName, activeLoop, pgOrders->getOIDType());
  ...
  RecordAttribute *projTupleR =
      new RecordAttribute(lineitemName, activeLoop, pgLineitem->getOIDType());
...
void tpchJoin1b(map<string, dataset> datasetCatalog, int predicate) {
  ...
  expressions::Expression *exprRightOID = new expressions::RecordProjection(
      pgLineitem->getOIDType(), rightArg, *projTupleR);
  //    expressions::Expression* exprOrderkey = new
...
void tpchJoin1c(map<string, dataset> datasetCatalog, int predicate) {
  ...
  jsonPipelined::JSONPlugin *pg =
      new jsonPipelined::JSONPlugin(&ctx, fileName, ordersDocType, linehint);
