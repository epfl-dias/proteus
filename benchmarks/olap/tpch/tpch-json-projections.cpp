...
void tpchLineitemProjection3Caching(map<string, dataset> datasetCatalog,
                                    int predicateVal, int aggregatesNo);
...
int main() {
  ...
  tpchLineitemProjection1(datasetCatalog, predicateMax);
  for (int i = 0; i < 5; i++) {
    cout << "[tpch-json-projections: ] Run " << i + 1 << endl;
    for (int i = 1; i <= 10; i++) {
...
void tpchLineitemProjection1(map<string, dataset> datasetCatalog,
                             int predicateVal) {
  Context &ctx = *prepareContext("tpch-json-projection1");
...
void tpchLineitemProjection2(map<string, dataset> datasetCatalog,
                             int predicateVal) {
  Context &ctx = *prepareContext("tpch-json-projection2");
  Catalog &rawCatalog = Catalog::getInstance();
...
void tpchLineitemProjection3Caching(map<string, dataset> datasetCatalog,
  ...
  //        expressions::Expression* lhsArg = new
  //        expressions::InputArgument(&attrType,
  //                0, projections);
