...
void tpchLineitemProjection3CachingAgg(map<string,dataset> datasetCatalog, int predicateVal, int aggregatesNo);
...
//	int predicateMax = L_ORDERKEY_MAX;
//	cout << "Preparing caches: Pred + Agg (&PM!)" << endl;
//	cout << "ReadPath() + CACHING (MATERIALIZING) INTO EFFECT: Pred+Agg" << endl;
//	tpchLineitemProjection3CachingPredAgg(datasetCatalog, predicateMax, 4);
//	for (int i = 0; i < 5; i++) {
//		cout << "[tpch-json-projections-cached: Schema + PredAgg ] Run " << i+1 << endl;
//			for (int i = 1; i <= 10; i++) {
//			double ratio = (i / (double) 10);
//			double percentage = ratio * 100;
//			int predicateVal = (int) ceil(predicateMax * ratio);
...
void tpchLineitemProjection3CachingAgg(map<string, dataset> datasetCatalog,
	...
	case 3: {
		...
		accs.push_back(MAX);
		expressions::Expression* outputExpr3 =
				new expressions::RecordProjection(linenumber->getOriginalType(),
						arg, *linenumber);
		outputExprs.push_back(outputExpr3);
		break;
	}
