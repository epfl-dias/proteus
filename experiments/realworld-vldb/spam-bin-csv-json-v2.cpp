...
void symantecBinCSVJSON2v1(map<string, dataset> datasetCatalog) {
  ...
  RadixJoin *joinBinJSON = new RadixJoin(*joinPred, selBin, selJSON, &ctx,
                                         joinLabel, *matLeft, *matRight);
  ...
  expressions::Expression *rightArg2 =
      new expressions::InputArgument(&recCSV, 1, argProjectionsRight2);
  expressions::Expression *rightPred2 = new expressions::RecordProjection(
  ...
  opt::Reduce *reduce =
      new opt::Reduce(accs, outputExprs, predRed, joinCSV, &ctx);
