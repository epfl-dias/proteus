...
void symantecJSON1Caching(map<string, dataset> datasetCatalog) {
  ...
  expressions::Expression *toMat4 =
      new expressions::RecordProjection(&monthType, matDay, *month);
...
void symantecJSON2v1(map<string, dataset> datasetCatalog) {
  ...
  expressions::Expression *argRed =
      new expressions::InputArgument(&rec, 0, argsReduce);
  /* Output: */
  vector<Monoid> accs;
  vector<expression_t> outputExprs;
  expressions::Expression *exprSize =
      new expressions::RecordProjection(size->getOriginalType(), argRed, *size);
  ...
  opt::Reduce *reduce = new opt::Reduce(accs, outputExprs, predRed, sel, &ctx);
  sel->setParent(reduce);
...
// select max((data->>'size')::int), count(*) from spamscoreiddates28m where
// (data->>'id')::int < 30000000 and (data->>'country_code') = 'US';
void symantecJSON3v1(map<string, dataset> datasetCatalog) {
  ...
  //    expressions::StringConstant(cityType);
...
void symantecJSON4v1(map<string, dataset> datasetCatalog) {
  ...
  accs.push_back(MAX);
  expressions::Expression *exprID =
      new expressions::RecordProjection(id->getOriginalType(), argRed, *id);
...
void symantecJSON6v1(map<string, dataset> datasetCatalog) {
  ...
  //            selCity, predExpr1);
  expressions::Expression *predicate =
      new expressions::EqExpression(selCountryCode, predExpr1);
