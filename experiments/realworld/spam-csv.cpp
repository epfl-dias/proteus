...
/* TODO Continue from this point on */
...
//	Select *sel = new Select(predicate1, scan);
//		scan->setParent(sel);
...
//
//
//			ReduceNoPred *reduce = new ReduceNoPred(SUM, outputExpr5, sel, &ctx);
//			sel->setParent(reduce);
////			Root *rootOp = new Root(reduce);
////			reduce->setParent(rootOp);
...
void symantecCSV5(map<string, dataset> datasetCatalog) {
	...
	expressions::Expression* predicateNum_ = new expressions::AndExpression(
				new BoolType(), predicate1, predicate2);
	expressions::Expression* predicateNum = new expressions::AndExpression(
					new BoolType(), predicateNum_, predicate2);
