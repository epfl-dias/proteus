...
#include "common/dclab_trace_lib.h"
// #include "cuda.h"
...

        {
            ...
            for (RawPipeline * p: pipelines) {
                nvtxRangePushA("pip");
                {
                    ...
                    dclab_trace::mark_a("open");
                    p->open();
                    dclab_trace::mark_a("/open");
                    p->consume(0);
                    dclab_trace::mark_a("close");
                    p->close();
                    dclab_trace::mark_a("/close");
                }
                nvtxRangePop();
...
TEST_F(MultiGPUTest, gpuTest3) {
    ...
    s = 0;
    {
        dclab_trace::mark_b("vec");
        time_block t("Tc: ");
        {
            time_block t("T: ");
            dclab_trace::mark_b("hpartv");
    ...
    s = 0;
    {
        dclab_trace::mark_b("simple");
        time_block t("Tc: ");
        {
            time_block t("T: ");
            dclab_trace::mark_b("hpart");
