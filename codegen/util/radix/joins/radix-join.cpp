...
void radix_cluster_nopadding(joins::tuple_t *outTuples,
                             joins::tuple_t *inTuples, size_t num_tuples, int R,
                             int D) {
  ...
  /* copy tuples to their corresponding clusters at appropriate offsets */
  {
    ...
    for (i = 0; i < ntuples; i++) {
      uint32_t idx = (uint32_t)(HASH_BIT_MODULO(input->key, M, R));
      //        cout << "[radix_cluster_nopadding: ] cluster: "<< idx <<" key? "
      //        << input->key << endl; if(R!=0)    {
      //            cout << input->key << endl;
      //            cnt++;
      //        }
      *dst[idx] = *input;
...
void bucket_chaining_join_prepare(const joins::tuple_t *const tuplesR,
  ...
  for (uint32_t i = 0; i < numR;) {
    ...
    // cout << "[K]: " << tuplesR[i].key << " [V]: " << tuplesR[i].payload <<
    // endl;
    (ht->next)[i] = (ht->bucket)[idx];
    (ht->bucket)[idx] = ++i; /* we start pos's from 1 instead of 0 */
  }
 ...
 * (when S side is partitioned)
 */
int64_t bucket_chaining_join_probe(const joins::relation_t *const R, HT *ht,
...
int *partitionHT(size_t num_tuples, joins::tuple_t *inTuples) {
  ...
  radix_cluster_nopadding(outTuples, inTuples, num_tuples, 0,
...
int64_t RJStepwise(joins::relation_t *relR, joins::relation_t *relS) {
  int64_t result = 0;
  uint32_t i;
  ...
  for (i = 0; i < (1 << NUM_RADIX_BITS); i++) {
    ...
    if (R_count_per_cluster[i] > 0 && S_count_per_cluster[i] > 0) {
      ...
      for (size_t j = 0; j < tmpS.num_tuples; j++) {
        result += bucket_chaining_join_probe(&tmpR, &(HT_per_cluster[i]),
                                             &(tmpS.tuples[j]));
      }
