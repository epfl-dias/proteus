...
class RawContext {
 public:
  ...
  LLVMContext &getLLVMContext() { return TheContext; }
  ...
  AllocaInst *createAlloca(BasicBlock *InsertAtBB, const string &VarName,
                           Type *varType);

  void CreateForLoop(const string &cond, const string &body, const string &inc,
                     const string &end, BasicBlock **cond_block,
  ...
  void CreateIfBlock(Function *fn, const string &if_name, BasicBlock **if_block,
                     BasicBlock *insert_before = NULL);

  BasicBlock *CreateIfBlock(Function *fn, const string &if_label,
                            BasicBlock *insert_before = NULL);
  ...
  StructType *getHashtableMetadataType() {
    ...
    StructType *metadataType =
        llvm::StructType::get(getLLVMContext(), types_htMetadata);
    return metadataType;
  }
