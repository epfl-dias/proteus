...
void HashRearrangeBuffered::produce() {
    ...
    Type   * int_type  = PointerType::get(Type::getInt32Ty(context->getLLVMContext()), 0);
...
void HashRearrangeBuffered::consume(RawContext* const context, const OperatorState& childState) {
    LLVMContext & llvmContext   = context->getLLVMContext();
    IRBuilder<> * Builder       = context->getBuilder    ();
    ...
    Value* mem_cache_cnt = ((GpuRawContext *) context)->getStateVar(cache_cnt_Var_id);
    ...
    Value* target_cache_cnt = Builder->CreateLoad(Builder->CreateInBoundsGEP(mem_cache_cnt, target));

    for (size_t i = 0 ; i < wantedFields.size() ; ++i){
        Value * cache_buffer = Builder->CreateLoad(Builder->CreateInBoundsGEP(mem_cache, context->createInt32(i)));
        Value * cache_buffer_cast = Builder->CreatePointerCast(cache_buffer, PointerType::get(wantedFields[i]->getExpressionType()->getLLVMType(llvmContext), 0));

        Value * el_ptr    = Builder->CreateInBoundsGEP(cache_buffer_cast, target_cache_cnt);
    ...
    Value * target_cache_cnt_next = Builder->CreateAdd(target_cache_cnt, context->createInt32(1));

    Value * cache_capacity_mask = Builder->CreateAnd(target_cache_cnt, cache_last_index);
    Value * flush_cond = Builder->CreateICmpUGE(cache_capacity_mask, cache_last_index);
    ...
    Builder->CreateStore(target_cache_cnt_next, Builder->CreateInBoundsGEP(mem_cache_cnt, target));
    Builder->CreateBr(mergecBB);

    Builder->SetInsertPoint(flushBB);
    Builder->CreateStore(Builder->CreateMul(cache_capacity, target), Builder->CreateInBoundsGEP(mem_cache_cnt, target));
    ...
    AllocaInst * mem_loop_offset  = context->CreateEntryBlockAlloca(F, "mem_loop_offset", int32_type);
    ...
    for (size_t i = 0 ; i < wantedFields.size() ; ++i){
        ...
        Value * in_cache_ptr    = Builder->CreateInBoundsGEP(cache_buffer_cast, cache_offset);
        ...
        argsV.push_back(context->createInt32(wantedFields[i]->getExpressionType()->getLLVMType(llvmContext)->getPrimitiveSizeInBits()/8));
        ...
        Value * el_block_ptr = Builder->CreateInBoundsGEP(in_block_ptr, loop_offset);
        Builder->CreateStore(Builder->CreateLoad(el_cache_ptr), el_block_ptr);
        Builder->CreateStore(Builder->CreateAdd(loop_offset, context->createInt32(1)), mem_loop_offset);
        Builder->CreateBr(loopCondBB);
    ...
    Builder->CreateStore(Builder->CreateAdd(indx, ConstantInt::get(oid_type, CACHE_CAP)), indx_addr);
    ...
    Value * cond = Builder->CreateICmpUGE(Builder->CreateAdd(indx, ConstantInt::get(oid_type, CACHE_CAP)), last_index);
...
void HashRearrangeBuffered::consume_flush1 () {
    ...
    flushingFunc1 = (*context)->createHelperFunction("flush1", std::vector<Type *>{}, std::vector<bool>{}, std::vector<bool>{});
    ...
    Value * numOfBucketsV = ConstantInt::get(target_type, numOfBuckets);
    Value * mem_target = context->CreateEntryBlockAlloca(F, "mem_target", int32_type);
    ...
    Value * cache_offset = Builder->CreateMul(cache_capacity, target);
    AllocaInst * mem_loop_offset  = context->CreateEntryBlockAlloca(F, "mem_loop_offset", int32_type);
    Value * loop_end = Builder->CreateSub(Builder->CreateLoad(Builder->CreateInBoundsGEP(mem_cache_cnt, target)), cache_offset);

    Builder->CreateStore(context->createInt32(0), mem_loop_offset);

    BasicBlock *InnerLoopCondBB  = BasicBlock::Create(llvmContext, "InnerLoopcond", F);
    BasicBlock *InnerLoopMainBB  = BasicBlock::Create(llvmContext, "InnerLoopmain", F);
    BasicBlock *InnerLoopMergeBB = BasicBlock::Create(llvmContext, "InnerLoopmerge", F);
    ...
    Builder->CreateStore(Builder->CreateAdd(target, context->createInt32(1)), mem_target);
    Builder->CreateBr(LoopCondBB);
...
void HashRearrangeBuffered::consume_flush(){
    ...
    flushingFunc2 = (*context)->createHelperFunction("flush", std::vector<Type *>{}, std::vector<bool>{}, std::vector<bool>{});
...
void HashRearrangeBuffered::open (RawPipeline * pip){
    ...
    for (size_t j = 0 ; j < wantedFields.size() ; ++j){
        ...
        posix_memalign((void**) &cache[j], 256, wfSizes[j] * CACHE_CAP * numOfBuckets);
    }
...
void HashRearrangeBuffered::close(RawPipeline * pip){
    ...
    ((void (*)(void *)) closingPip1->getCompiledFunction(flushingFunc1))(pip->getState());
    ((void (*)(void *)) closingPip->getCompiledFunction(flushingFunc2))(pip->getState());
