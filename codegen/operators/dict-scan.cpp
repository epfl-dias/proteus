...
public:
    ...
    bool operator==(const DictMatchIter &other){
...
DictMatchIter DictScan::begin() const{
    return DictMatchIter{*this, regex};
}

DictMatchIter DictScan::end  () const{
    return DictMatchIter{*this};
}

extern "C" void initDictScan(const DictScan * dict,
        DictMatchIter * curr, DictMatchIter * end){
...
extern "C" int32_t getDictScan(DictMatchIter * begin){
    return **begin;
}

extern "C" void nextDictScan(DictMatchIter * begin){
    ++(*begin);
}
...
void DictScan::produce(){
    ...
    auto initDictScan  = FunctionType::get(void_type, {charPtrType, iterPtrType, iterPtrType}, false);
    auto finitDictScan = Function::Create(initDictScan, Function::ExternalLinkage, "initDictScan", context->getModule());

    auto hasNextDictScan  = FunctionType::get(bool_type, {iterPtrType, iterPtrType}, false);
    auto fhasNextDictScan = Function::Create(hasNextDictScan, Function::ExternalLinkage, "hasNextDictScan", context->getModule());

    auto getDictScan  = FunctionType::get(i32_type, {iterPtrType}, false);
    auto fgetDictScan = Function::Create(getDictScan, Function::ExternalLinkage, "getDictScan", context->getModule());

    auto nextDictScan  = FunctionType::get(void_type, {iterPtrType}, false);
    auto fnextDictScan = Function::Create(nextDictScan, Function::ExternalLinkage, "nextDictScan", context->getModule());
...
void DictScan::consume(GpuRawContext* const context, const OperatorState& childState){
    throw runtime_error(string("unexpected call to DistScan::consume (DictScan operator has no children)"));
}
