...
void GpuHashGroupByChained::consume(RawContext *const context,
...
Value *GpuHashGroupByChained::hash(const std::vector<expression_t> &exprs,
...
void GpuHashGroupByChained::generate_build(RawContext *const context,
                                           const OperatorState &childState) {
  ...
  Builder->CreateAtomicRMW(
      ...
      eochain, llvm::AtomicOrdering::Monotonic);
  ...
  Builder->CreateAtomicRMW(llvm::AtomicRMWInst::BinOp::Xchg, inv_ptr,
  ...
  // index
  old_cnt = Builder->CreateAtomicRMW(
      llvm::AtomicRMWInst::BinOp::Add, out_cnt,
      ConstantInt::get(out_cnt->getType()->getPointerElementType(), 1),
      llvm::AtomicOrdering::Monotonic);
  Builder->CreateStore(old_cnt, mem_idx);
...
// void GpuHashGroupByChained::generate_build(RawContext* const context, const
...
//     Value * out_cnt = ((const GpuRawContext *)
//     context)->getStateVar(cnt_param_id); out_cnt ->setName(opLabel +
//     "_cnt_ptr");

//     Value * head_ptr = ((const GpuRawContext *)
//     context)->getStateVar(head_param_id); head_ptr->setName(opLabel +
//     "_head_ptr");
...
//     // ExpressionHasherVisitor exphasher{context, childState};
//     // Value * hash = kexpr.accept(exphasher).value;
//     Value * hash = GpuHashGroupByChained::hash(key_expr, context,
//     childState);

//     Value * eochain   = ConstantInt::get((IntegerType *)
//     head_ptr->getType()->getPointerElementType(), ~((size_t) 0));
//     //current = head[hash(key)]
//     // Value * current =
//     Builder->CreateAlignedLoad(Builder->CreateInBoundsGEP(head_ptr, hash),
//     context->getSizeOf(head_ptr->getType()->getPointerElementType())); Value
//     * head_w_hash_ptr = Builder->CreateInBoundsGEP(head_ptr, hash);
//     // Value * current =
...
//     // AllocaInst *mem_current = context->CreateEntryBlockAlloca(TheFunction,
//     "mem_current", current->getType());
//     // Builder->CreateStore(current, mem_current);

//     BasicBlock * saveBB = Builder->GetInsertBlock();

//     Builder->SetInsertPoint(context->getCurrentEntryBlock());
//     AllocaInst *mem_idx     = context->CreateEntryBlockAlloca(TheFunction,
//     "mem_idx", eochain->getType()); Builder->CreateStore(eochain, mem_idx);

//     Builder->SetInsertPoint(saveBB);

//     // AllocaInst *mem_written = context->CreateEntryBlockAlloca(TheFunction,
//     "mem_written", v_false->getType());
//     // Builder->CreateStore(v_false, mem_written);
...
//     for (size_t i = 0 ; i < out_param_ids.size() ; ++i) {
//         Value * out_ptr = ((const GpuRawContext *)
//         context)->getStateVar(out_param_ids[i]); out_ptr->setName(opLabel +
//         "_data" + std::to_string(i) + "_ptr");
//         // out_ptrs.push_back(out_ptr);
...
//     //if mem_idx == -1:
//     Value * alloc = Builder->CreateICmpEQ(Builder->CreateLoad(mem_idx),
//     eochain);

//     BasicBlock *allocBB      = BasicBlock::Create(llvmContext, "alloc"     ,
//     TheFunction); BasicBlock *allocAfterBB = BasicBlock::Create(llvmContext,
//     "allocAfter", TheFunction); Builder->CreateCondBr(alloc, allocBB,
//     allocAfterBB);

//     BasicBlock *deallocBB      = BasicBlock::Create(llvmContext, "dealloc" ,
//     TheFunction); BasicBlock *deallocAfterBB =
//     BasicBlock::Create(llvmContext, "deallocAfter", TheFunction);
//     Builder->SetInsertPoint(context->getEndingBlock());
//     Value * dealloc = Builder->CreateICmpNE(Builder->CreateLoad(mem_idx),
//     eochain); Builder->CreateCondBr(dealloc, deallocBB, deallocAfterBB);

//     Builder->SetInsertPoint(deallocBB);

//     Value * inv_ptr = Builder->CreateInBoundsGEP(((const GpuRawContext *)
//     context)->getStateVar(out_param_ids[0]), std::vector<Value
...
//     // Builder->CreateAtomicRMW(llvm::AtomicRMWInst::BinOp::Xchg,
//     //     Builder->CreateInBoundsGEP(context->getStateVar(out_param_ids[0]),
//     std::vector<Value *>{alloced_idx, context->createInt32(0)}),
//     //     eochain,
//     //     llvm::AtomicOrdering::Monotonic);

//     for (size_t i = 1 ; i < out_param_ids.size() ; ++i) {
//         Value * out_ptr = ((const GpuRawContext *)
//         context)->getStateVar(out_param_ids[i]);

//         Value * out_ptr_i = Builder->CreateInBoundsGEP(out_ptr, alloced_idx);
//         Builder->CreateStore(out_vals[i], out_ptr_i, true); //Aligned: ,
//         packet_widths[i]/8
//     }
...
//     // BasicBlock *InitCondBB  = BasicBlock::Create(llvmContext,
//     "setHeadCond", TheFunction); BasicBlock *InitThenBB  =
//     BasicBlock::Create(llvmContext, "setHead", TheFunction); BasicBlock
//     *InitMergeBB = BasicBlock::Create(llvmContext, "cont"   , TheFunction);

//     BasicBlock *ThenBB  = BasicBlock::Create(llvmContext, "chainFollow"    ,
//     TheFunction); BasicBlock *MergeBB = BasicBlock::Create(llvmContext,
//     "cont"           , TheFunction); BasicBlock *AfterMergeBB =
//     BasicBlock::Create(llvmContext, "groupbyend"     , TheFunction);
...
//     AllocaInst *mem_current = context->CreateEntryBlockAlloca(TheFunction,
//     "mem_current", Builder->CreateExtractValue(old_current, 0)->getType());
//     Builder->CreateStore(Builder->CreateExtractValue(old_current, 0),
//     mem_current);

//     Value * suc = Builder->CreateICmpEQ(Builder->CreateLoad(mem_current),
//     eochain);
...
//     BasicBlock *BucketFoundBB = BasicBlock::Create(llvmContext,
//     "BucketFound", TheFunction); BasicBlock *ContFollowBB  =
//     BasicBlock::Create(llvmContext, "ContFollow" , TheFunction);

//     Value * next_bucket_ptr = Builder->CreateInBoundsGEP(((const
//     GpuRawContext *) context)->getStateVar(out_param_ids[1]),
//     Builder->CreateLoad(mem_current)); Value * next_bucket     =
//     Builder->CreateLoad(next_bucket_ptr, true, "next_bucket"); //Aligned: ,
//     packet_widths[1]/8
...
//     // Value * str = UndefValue::get(((const GpuRawContext *)
//     context)->getStateVar(out_param_ids[0])->getType()->getPointerElementType());
//     // str = Builder->CreateInsertValue(str, Builder->CreateLoad(mem_idx),
//     0);
//     // Value * inv_ptr = Builder->CreateInBoundsGEP(((const GpuRawContext *)
//     context)->getStateVar(out_param_ids[0]), std::vector<Value *>{old_cnt,
//     context->createInt32(0)});
...
//     Builder->SetInsertPoint(ContFollowBB);
//     Value * next =
//     Builder->CreateAtomicCmpXchg(Builder->CreateInBoundsGEP(((const
//     GpuRawContext *) context)->getStateVar(out_param_ids[0]),
//     std::vector<Value *>{Builder->CreateLoad(mem_current),
//     context->createInt32(0)}),
//                                                 eochain,
//                                                 old_cnt,
//                                                 llvm::AtomicOrdering::Monotonic,
...
//     Value * chain_cond =
//     Builder->CreateICmpNE(Builder->CreateExtractValue(next, 0), eochain);
//     // Value * chain_cond2 =
//     Builder->CreateICmpNE(Builder->CreateLoad(mem_current), old_cnt);
...
// void GpuHashGroupByChained::generate_probe(RawContext* const context, const
// OperatorState& childState) {
//     IRBuilder<>    *Builder     = context->getBuilder();
//     LLVMContext    &llvmContext = context->getLLVMContext();
//     Function       *TheFunction = Builder->GetInsertBlock()->getParent();
...
//     Argument * head_ptr = ((const GpuRawContext *)
//     context)->getStateVar(probe_head_param_id); head_ptr->setName(opLabel +
//     "_head_ptr");
...
//     BasicBlock *CondBB  = BasicBlock::Create(llvmContext, "chainFollowCond",
//     TheFunction); BasicBlock *ThenBB  = BasicBlock::Create(llvmContext,
//     "chainFollow"    , TheFunction); BasicBlock *MergeBB =
//     BasicBlock::Create(llvmContext, "cont"           , TheFunction);
...
//         Argument * in_ptr = ((const GpuRawContext *)
//         context)->getStateVar(in_param_ids[i]); if (in_param_ids.size() !=
//         1){
...
//         in_ptrs.push_back(Builder->CreateInBoundsGEP(in_ptr,
//         Builder->CreateLoad(mem_current)));
//         in_vals.push_back(Builder->CreateAlignedLoad(in_ptrs.back(),
...
//     BasicBlock *MatchThenBB  = BasicBlock::Create(llvmContext,
//     "matchChainFollow"    , TheFunction);
...
//     // Triggering parent
//     OperatorState* newState = new
//     OperatorState(*this,childState.getBindings());
...
void GpuHashGroupByChained::open(RawPipeline *pip) {
  ...
  int32_t *first = (int32_t *)RawMemoryManager::mallocGpu(sizeof(int32_t) *
                                                          (1 << hash_bits));
