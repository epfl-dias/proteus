...
void GpuSort::flush_sorted(){
    ...
    // Builder->CreateCall(p, Builder->CreateBitCast(mem_ptr, Type::getInt8PtrTy(llvmContext)));

    variableBindings[brec] = RawValueMemory{mem_mem_ptr, context->createFalse()};

    AllocaInst * cnt_mem = context->CreateEntryBlockAlloca(F, "activeCnt_mem", oid_type);
