...
void GpuHashJoinChained::consume(RawContext *const context,
...
Value *GpuHashJoinChained::hash(expression_t exprs, RawContext *const context,
                                const OperatorState &childState) {
...
void GpuHashJoinChained::generate_build(RawContext *const context,
                                        const OperatorState &childState) {
  ...
  Value *old_head =
      Builder->CreateAtomicRMW(llvm::AtomicRMWInst::BinOp::Xchg,
                               ...
                               old_cnt, llvm::AtomicOrdering::Monotonic);
...
void GpuHashJoinChained::generate_probe(RawContext *const context,
                                        const OperatorState &childState) {
  IRBuilder<> *Builder = context->getBuilder();
  LLVMContext &llvmContext = context->getLLVMContext();
  Function *TheFunction = Builder->GetInsertBlock()->getParent();
  ...
  AllocaInst *mem_counter = context->CreateEntryBlockAlloca(
      TheFunction, "mem_counter", Type::getInt32Ty(llvmContext));
  Builder->CreateStore(ConstantInt::get(Type::getInt32Ty(llvmContext), 0),
                       mem_counter);
  ...
  Builder->CreateStore(
      Builder->CreateAdd(ConstantInt::get(Type::getInt32Ty(llvmContext), 1),
                         Builder->CreateLoad(mem_counter)),
      mem_counter);
  ...
  for (const GpuMatExpr &mexpr : probe_mat_exprs) {
    ...
    {  // NOTE: Is there a better way ?
      ...
      if (!pr_oid_type) {
        string error_msg(
            "[GpuHashJoinChained: ] Only primitive OIDs are supported.");
        LOG(ERROR) << error_msg;
...
void GpuHashJoinChained::open_build(RawPipeline *pip) {
  std::cout << "GpuHashJoinChained::open::build_" << pip->getGroup()
            << std::endl;
...
void GpuHashJoinChained::close_build(RawPipeline *pip) {
  std::cout << "GpuHashJoinChained::close::build_" << pip->getGroup()
            << std::endl;
  int32_t h_cnt;
...
void GpuHashJoinChained::close_probe(RawPipeline *pip) {
  std::cout << "GpuHashJoinChained::close::probe_" << pip->getGroup()
            << std::endl;
