...
void HashGroupByChained::consume(RawContext* const context, const OperatorState& childState) {
...
void HashGroupByChained::buildHashTableFormat(){
    ...
    Type *t_head_ptr = PointerType::get(ArrayType::get(int32_type , 1 << hash_bits), /* address space */ 0);
    head_param_id = context->appendStateVar(
        ...
        [=](llvm::Value *){
            ...
            Value * mem_acc = context->allocateStateVar(ArrayType::get(int32_type , 1 << hash_bits));

            context->CodegenMemset(mem_acc, context->createInt8(-1), (1 << hash_bits) * sizeof(int32_t));
...
void HashGroupByChained::generate_build(RawContext* const context, const OperatorState& childState) {
    IRBuilder<>    *Builder     = context->getBuilder();
    ...
    Value * head_w_hash_ptr = Builder->CreateInBoundsGEP(head_ptr, {context->createInt64(0), hash});
    ...
    //     eochain,
    //     llvm::AtomicOrdering::Monotonic);
    Builder->CreateStore(eochain, Builder->CreateBitCast(Builder->CreateInBoundsGEP(context->getStateVar(out_param_ids[0]), old_cnt), PointerType::getInt32PtrTy(llvmContext)));
    ...
    //                                             llvm::AtomicOrdering::Monotonic,
    //                                             llvm::AtomicOrdering::Monotonic), 0);
    Value * next = Builder->CreateLoad(Builder->CreateInBoundsGEP(((const GpuRawContext *) context)->getStateVar(out_param_ids[0]), std::vector<Value *>{current, context->createInt32(0)}));
    ...
    Value * n_ptr = Builder->CreateInBoundsGEP(((const GpuRawContext *) context)->getStateVar(out_param_ids[0]), tmp);
    ...
    context->gen_if(RawValue{Builder->CreateICmpEQ(new_next, eochain), context->createFalse()})([=](){
    ...
    Builder->CreateCondBr(Builder->CreateICmpEQ(new_next, eochain), MergeBB, ThenBB);
...
void HashGroupByChained::open(RawPipeline * pip){
    ...
    // int32_t * first = (int32_t *) RawMemoryManager::mallocPinned(sizeof(int32_t  ) * (1 << hash_bits));
