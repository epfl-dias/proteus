...
void RadixJoinBuild::initializeState(){
    ...
    rel.mem_relation_id = context->appendStateVar(
        ...
        [=](llvm::Value *){
            ...
            Value * mem_rel = Builder->CreateCall(getMemChunk, 
                                            context->createSizeT(size));
...
void RadixJoinBuild::consume(   GpuRawContext* const context    ,
    ...
    if(!cached){
        ...
        Value *val_arena                = Builder->CreateLoad(context->getStateVar(rel.mem_relation_id));
        Value *offsetInArena            = Builder->CreateLoad(context->getStateVar(rel.mem_offset_id));
        Value *offsetPlusPayload        = Builder->CreateAdd (offsetInArena, val_payloadSize);
        Value *arenaSize                = Builder->CreateLoad(context->getStateVar(rel.mem_size_id));
        Value *val_tuplesNo             = Builder->CreateLoad(context->getStateVar(rel.mem_tuplesNo_id));
        ...
        Builder->CreateStore(val_newArenaVoidPtr, context->getStateVar(rel.mem_relation_id));
        Value* val_size = Builder->CreateLoad(context->getStateVar(rel.mem_size_id));
...
//          }
        ...
        Value *offsetInHT = Builder->CreateLoad(context->getStateVar(ht.mem_offset_id));
...
//      Value *offsetPlusPayloadPtr = Builder->CreateAdd(offsetPlusKey,val_payloadPtrSize);
        ...
        Builder->CreateStore(offsetPlusPayload, context->getStateVar(rel.mem_offset_id));
        Builder->CreateStore(offsetPlusKVPair, context->getStateVar(ht.mem_offset_id));
        val_tuplesNo = Builder->CreateAdd(val_tuplesNo, context->createInt64(1));
        Builder->CreateStore(val_tuplesNo, context->getStateVar(rel.mem_tuplesNo_id));
        Builder->CreateStore(val_tuplesNo, context->getStateVar(ht.mem_tuplesNo_id));
    } else {
...
void RadixJoin::consume(RawContext* const context,
            const OperatorState& childState){
    assert(false && "Function should not be called! Are RadixJoinBuilders correctly set as children of this operator?");
}
 ...
 */
Value *RadixJoinBuild::radix_cluster_nopadding(Value * mem_tuplesNo, Value * mem_kv_id) const    {
    LLVMContext& llvmContext = context->getLLVMContext();
    RawCatalog& catalog = RawCatalog::getInstance();
    Function *F = context->getGlobalFunction();
    IRBuilder<> *Builder = context->getBuilder();
...
void RadixJoin::runRadix() const    {
    ...
    size_t clusterCountS_id = context->appendStateVar(
        ...
        "clusterCountS"
    ); //FIXME: read-only, we do not even have to maintain it as state variable, but where do we get pip from ?
