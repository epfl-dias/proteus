...
Project::Project(vector<expression_t> outputExprs,
		...
		RawContext         *context) :
		UnaryRawOperator(child), outputExprs(outputExprs), relName(relName), context(context) {
}
...
void Project::consume(RawContext* const context, const OperatorState& childState) {
...
void Project::generate(RawContext* const context,
	...
	AllocaInst * local_mem_oid = context->CreateEntryBlockAlloca("oid", local_oid->getType());
	AllocaInst * local_mem_cnt = context->CreateEntryBlockAlloca("cnt", local_oid->getType());
	...
	Value * next_oid = Builder->CreateAdd(Builder->CreateLoad(local_mem_oid), context->createInt32(0));
