...
BinaryColPlugin::BinaryColPlugin(Context *const context, string fnamePrefix,
  ...
  for (it = wantedFields.begin(); it != wantedFields.end(); it++) {
    ...
    std::vector<mem_file> file_parts = StorageManager::getOrLoadFile(
    ...
    if ((*it)->getOriginalType()->getTypeID() == STRING) {
      ...
      if (dictionaryFd == -1) {
        string error_msg =
            string("[Binary Col Plugin]: Opening column dictionary failed");
        LOG(ERROR) << error_msg;
        throw runtime_error(error_msg);
      }
...
//        fields"); LOG(ERROR) << error_msg; throw runtime_error(error_msg);
//    }
//
 ...
 * buffers we need, the val. size, etc. */
...
//
//        AllocaInst *offsetMem =
//        context->CreateEntryBlockAlloca(F,currPosVar,int64Type); AllocaInst
//        *bufMem = context->CreateEntryBlockAlloca(F,currBufVar,charPtrType);
//
...
//        AllocaInst *mem_bufPtr =
//        context->CreateEntryBlockAlloca(F,string("mem_bufPtr"),charPtrType);
//        Value* val_bufPtr = ConstantInt::get(llvmContext,
//        APInt(64,((uint64_t)buf[cnt])));
//        //i8*
...
//    AllocaInst *mem_itemCtr = context->CreateEntryBlockAlloca(F, itemCtrVar,
//    int64Type); Builder->CreateStore(val_itemCtr,mem_itemCtr);
...
void BinaryColPlugin::init() {
  ...
  for (it = wantedFields.begin(); it != wantedFields.end(); it++) {
    ...
    AllocaInst *offsetMem =
        context->CreateEntryBlockAlloca(F, currPosVar, int64Type);
    AllocaInst *bufMem =
        context->CreateEntryBlockAlloca(F, currBufVar, charPtrType);
    ...
    AllocaInst *mem_bufPtr =
        context->CreateEntryBlockAlloca(F, string("mem_bufPtr"), charPtrType);
    Value *val_bufPtr =
        ConstantInt::get(llvmContext, APInt(64, ((uint64_t)buf[cnt])));
    // i8*
    Value *unshiftedPtr = Builder->CreateIntToPtr(val_bufPtr, charPtrType);
    Builder->CreateStore(unshiftedPtr, bufMem);
    NamedValuesBinaryCol[currBufVar] = bufMem;
    ...
    if (wantedFields.at(cnt)->getOriginalType()->getTypeID() == STRING) {
      ...
      AllocaInst *dictMem =
          context->CreateEntryBlockAlloca(F, currDictVar, charPtrType);

      // Typical way to pass a pointer via the LLVM API
      AllocaInst *mem_dictPtr = context->CreateEntryBlockAlloca(
          F, string("mem_dictPtr"), charPtrType);
      Value *val_dictPtr = ConstantInt::get(
          llvmContext, APInt(64, ((uint64_t)dictionariesBuf[cnt])));
      // i8*
    ...
    //            cout << "[Binary Col. Plugin:] Register (bin col.) in cache"
    //            << endl; CachingService& cache =
    //            CachingService::getInstance(); bool fullRelation = true;
    //
    //            CacheInfo info;
  ...
  // cout << "[BinaryColPlugin: ] Initialization Successful for " << fnamePrefix
  // << endl; Global item counter
  Value *val_itemCtr = context->createInt64(0);
  AllocaInst *mem_itemCtr =
      context->CreateEntryBlockAlloca(F, itemCtrVar, int64Type);
  ...
  ProteusValueMemory mem_projection;
  {
    ...
    if (it == binProjections.end()) {
      string error_msg =
          ...
          pathVar;
      //            for (it = binProjections.begin(); it !=
      //            binProjections.end();
      //                    it++) {
      //                RecordAttribute attr = it->first;
      //                cout << attr.getRelationName() << "_" <<
      //                attr.getAttrName()
      //                        << endl;
      //            }
...
ProteusValue BinaryColPlugin::hashValue(ProteusValueMemory mem_value,
  ...
  switch (type->getTypeID()) {
    ...
    case STRING: {
      ...
      throw runtime_error(
          string("[BinaryColPlugin: ] String datatypes not supported yet"));
    }
    case FLOAT: {
    ...
    case SET:
      ...
      throw runtime_error(
          string("[BinaryColPlugin: ] Cannot contain collections"));
    case RECORD:
      LOG(ERROR)
          << "[BinaryColPlugin: ] Cannot contain record-valued attributes";
...
void BinaryColPlugin::scan(const ::Operator &producer) {
  ...
  for (vector<RecordAttribute *>::iterator it = wantedFields.begin();
    ...
    switch ((*it)->getOriginalType()->getTypeID()) {
      ...
      case LIST:
      case SET:
        LOG(ERROR) << "[BINARY COL PLUGIN: ] Binary col files do not contain "
                      "collections";
        throw runtime_error(
            string("[BINARY COL PLUGIN: ] Binary col files do not contain "
                   "collections"));
      case RECORD:
        LOG(ERROR) << "[BINARY COL PLUGIN: ] Binary col files do not contain "
                      "record-valued attributes";
        throw runtime_error(
            string("[BINARY COL PLUGIN: ] Binary col files do "
                   "not contain record-valued attributes"));
      default:
