...
expr_dir="/scratch2/sig20_pelago/exper_elastic"
...
do
	...
	do
		...
		do
			...
			echo "#######################################################"
			#cow_query_expr_100="/scratch/raza/cidr20/opt/pelago/htap-server-fork-100-snapshot  --num-oltp-clients=36 --num-olap-clients=36 --plan-json=/scratch/raza/cidr20/plan.json"
			cmd_a="$EXE_DIR/htap-server-circular-master --num-oltp-clients=$num_oltp_clients --num-olap-clients=$qs  --ch-scale-factor=$SF"
			cmd="$cmd_a | tee $expr_dir/htap-server-local-$qqq-query_$EXPR_NUM"
			echo "$cmd_a"
			$cmd_a 2>&1 | tee $expr_dir/htap-server-remote--$SF-$qqq-query_$EXPR_NUM
