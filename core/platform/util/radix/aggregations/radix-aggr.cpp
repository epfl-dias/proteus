 ...
 * clusters.
 */
void radix_cluster_nopadding(agg::relation_t *outRel, agg::relation_t *inRel,
                             int R, int D) {
  agg::tuple_t **dst;
  agg::tuple_t *input;
  /* tuple_t ** dst_end; */
...
void radix_cluster_nopadding(agg::tuple_t *outTuples, agg::tuple_t *inTuples,
                             size_t num_tuples, int R, int D) {
  agg::tuple_t **dst;
  agg::tuple_t *input;
  /* tuple_t ** dst_end; */
  uint32_t *tuples_per_cluster;
 ...
 * (when S side is partitioned)
 */
int64_t bucket_chaining_agg_probe(const agg::relation_t *const R, HT *ht,
  ...
  for (i = 0; i < (1 << NUM_RADIX_BITS); i++) {
    ...
    if (R_count_per_cluster[i] > 0 && S_count_per_cluster[i] > 0) {
      ...
      for (size_t j = 0; j < tmpS.num_tuples; j++) {
        result += bucket_chaining_agg_probe(&tmpR, &(HT_per_cluster[i]),
                                            &(tmpS.tuples[j]));
      }
