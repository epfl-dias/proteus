...
static void steamGC(global_conf::IndexVal* idx_ptr, txn::TxnTs gmin) {
  ...
  auto& delta_list =
      reinterpret_cast<TaggedDeltaDataPtr<T>&>(idx_ptr->delta_list);

  TaggedDeltaDataPtr<T> cleanable;
  ...
  if (global_conf::ConcurrencyControl::is_readable(idx_ptr->t_min, gmin)) {
    ...
    const auto* versionBase =
        VersionChain<MV_RecordList_Full>::get_readable_ver(gmin, delta_list,
                                                           false);
...
void MV_RecordList_Full::gc(global_conf::IndexVal* idx_ptr, txn::TxnTs gmin) {
  steamGC<version_t>(idx_ptr, gmin);
}
...
std::vector<MV_RecordList_Full::version_t*> MV_RecordList_Full::create_versions(
  ...
  auto deltaDataPtr =
      deltaStore.allocate(ver_size + sizeof(MV_RecordList_Full::version_t) +
                              256 + alignof(MV_RecordList_Full::version_t),
                          partition_id);
  ...
  auto* ver = new (base_ptr) MV_RecordList_Full::version_t(
      idx_ptr->t_min, 0,
      base_ptr + alignof(MV_RecordList_Full::version_t) + 64 +
...
//
//  auto ver_data_size =
//  MV_RecordList_Partial::version_t::get_partial_mask_size(
//      attribute_widths, ver_offsets, attr_mask, col_idx, num_cols);
//
//  auto offset_arr_sz = attr_mask.count() * sizeof(uint16_t);
//
//  //  auto* ver =
//  (MV_RecordList_Partial::version_t*)deltaStore.insert_version(
...
//      sizeof(MV_RecordList_Partial::version_t), partition_id);
//  auto* base_ptr = static_cast<char*>(deltaDataPtr.get_ptr());
//
//  auto* ver = new (base_ptr) MV_RecordList_Partial::version_t(
//      idx_ptr->t_min, 0, base_ptr + sizeof(MV_RecordList_Partial::version_t));
//
//  // point next version to previous head, regardless it is valid or not.
//  if (idx_ptr->delta_list.ptr() != nullptr) {
//    ver->next = idx_ptr->delta_list;
//  }
//
//  // update pointer on delta_list (which is inside index) to point to latest
//  // version.
...
//    // The problem with before-images is that you have to traverse the
//    snapshot
//    // until the visible part, no matter the required attribute is there or
//    not.
//
//    // before-images copy
//    // The following, undo the record image, for the set of required
//    attributes
//    // only.
