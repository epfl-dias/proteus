...
void BinaryRowPlugin::init() {
  ...
  AllocaInst *offsetMem = context->CreateEntryBlockAlloca(
      F, std::string(posVar), Type::getInt64Ty(llvmContext));
  AllocaInst *bufMem =
      context->CreateEntryBlockAlloca(F, std::string(bufVar), charPtrType);
  AllocaInst *fsizeMem = context->CreateEntryBlockAlloca(
      F, std::string(fsizeVar), Type::getInt64Ty(llvmContext));
...
ProteusValueMemory BinaryRowPlugin::readPath(string activeRelation,
  ...
  ProteusValueMemory mem_projection;
  {
    ...
    if (it == binProjections.end()) {
      string error_msg =
          string("[Binary Row plugin - readPath ]: Unknown variable name ") +
          pathVar;
      LOG(ERROR) << error_msg;
...
llvm::Value *BinaryRowPlugin::getValueSize(ProteusValueMemory mem_value,
  ...
  switch (type->getTypeID()) {
    ...
    case STRING: {
      string error_msg =
          string("[Binary Row Plugin]: Strings not supported yet");
      LOG(ERROR) << error_msg;
      throw runtime_error(error_msg);
    }
    case BAG:
    ...
    case SET: {
      string error_msg =
          string("[Binary Row Plugin]: Cannot contain collections");
      LOG(ERROR) << error_msg;
      throw runtime_error(error_msg);
    }
    case RECORD: {
      string error_msg = string("[Binary Row Plugin]: Cannot contain records");
      LOG(ERROR) << error_msg;
      throw runtime_error(error_msg);
    }
    default: {
  ...
  for (vector<RecordAttribute *>::iterator it = wantedFields.begin();
    ...
    for (; (*iterSchema)->getAttrNo() < attr.getAttrNo(); iterSchema++) {
      switch ((*iterSchema)->getOriginalType()->getTypeID()) {
        ...
        case LIST:
        case SET:
          LOG(ERROR) << "[BINARY ROW PLUGIN: ] Binary row files do not contain "
                        "collections";
          throw runtime_error(
              string("[BINARY ROW PLUGIN: ] Binary row files do "
    ...
    switch ((*it)->getOriginalType()->getTypeID()) {
      ...
      case SET:
        LOG(ERROR) << "[BINARY ROW PLUGIN: ] Binary row files do not contain "
                      "collections";
        throw runtime_error(
            string("[BINARY ROW PLUGIN: ] Binary row files do not contain "
                   "collections"));
      case RECORD:
        LOG(ERROR) << "[BINARY ROW PLUGIN: ] Binary row files do not contain "
                      "record-valued attributes";
    ...
    switch ((*iterSchema)->getOriginalType()->getTypeID()) {
      ...
      case SET:
        ...
        throw runtime_error(string(
            "[CSV PLUGIN: ] Binary row files do not contain collections"));
      case RECORD:
        LOG(ERROR) << "[BINARY ROW PLUGIN: ] Binary row files do not contain "
                      "record-valued attributes";
        throw runtime_error(
            string("[CSV PLUGIN: ] Binary row files do not "
                   "contain record-valued attributes"));
      default:
        LOG(ERROR) << "[BINARY ROW PLUGIN: ] Unknown datatype";
        throw runtime_error(string("[BINARY ROW PLUGIN: ] Unknown datatype"));
    }
