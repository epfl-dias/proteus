...
#include "binary-internal-plugin.hpp"
...
BinaryInternalPlugin::BinaryInternalPlugin(
  ...
  if (fieldsNumber <= 0) {
    string error_msg =
        string("[Binary Internal Plugin]: Invalid number of fields");
    LOG(ERROR) << error_msg;
    throw runtime_error(error_msg);
  }

  mem_buffer =
      context->CreateEntryBlockAlloca(F, string("binBuffer"), charPtrType);
  ...
  mem_cnt =
      context->CreateEntryBlockAlloca(F, string("currTupleNo"), int64Type);
  ...
  //    cout << "Internal Binary PG creation - " << info.objectTypes.size() << "
  //    fields" << endl; payloadPtrType->dump(); cout << endl;
}
...
void BinaryInternalPlugin::scan(const ::Operator &producer) {
  ...
  for (vector<RecordAttribute *>::iterator it = OIDs.begin(); it != OIDs.end();
    ...
    switch (attr.getOriginalType()->getTypeID()) {
      ...
      case SET:
        LOG(ERROR)
            << "[BinaryInternalPlugin: ] Binary row files do not contain "
               "collections";
        throw runtime_error(
            string("[BinaryInternalPlugin: ] Binary row files do "
...
ProteusValueMemory BinaryInternalPlugin::readPath(string activeRelation,
  ...
  ProteusValueMemory mem_valWrapper;
  {
    ...
    if (it == binProjections.end()) {
      string error_msg =
          string("[BinaryInternalPlugin - readPath ]: Unknown variable name ") +
          pathVar;
      LOG(ERROR) << error_msg;
      throw runtime_error(error_msg);
    }
...
ProteusValue BinaryInternalPlugin::hashValue(ProteusValueMemory mem_value,
  ...
  switch (type->getTypeID()) {
    ...
    case STRING: {
      LOG(ERROR)
          << "[BinaryInternalPlugin: ] String datatypes not supported yet";
    ...
    case SET: {
      LOG(ERROR)
          << "[BinaryInternalPlugin: ] Collection datatypes not supported yet";
      throw runtime_error(string(
          "[BinaryInternalPlugin: ] Collection datatypes not supported yet"));
    }
    case RECORD: {
      LOG(ERROR) << "[BinaryInternalPlugin: ] Record-valued datatypes not "
                    "supported yet";
      throw runtime_error(
          string("[BinaryInternalPlugin: ] Record-valued datatypes not "
                 "supported yet"));
    }
    default: {
...
llvm::Value *BinaryInternalPlugin::getValueSize(ProteusValueMemory mem_value,
  ...
  switch (type->getTypeID()) {
    ...
    case FLOAT: {
      ...
      // return context->createInt32(explicitType->getPrimitiveSizeInBits() /
      // 8);
    }
    case STRING: {
      string error_msg =
          string("[BinaryInternalPlugin]: Strings not supported yet");
      LOG(ERROR) << error_msg;
      throw runtime_error(error_msg);
    }
    case BAG:
    ...
    case SET: {
      string error_msg =
          string("[BinaryInternalPlugin]: Collections not supported yet");
      LOG(ERROR) << error_msg;
      throw runtime_error(error_msg);
    }
    case RECORD: {
      string error_msg =
          string("[BinaryInternalPlugin]: Records not supported yet");
      LOG(ERROR) << error_msg;
      throw runtime_error(error_msg);
    }
    default: {
...
void BinaryInternalPlugin::readAsStringLLVM(
  ...
  AllocaInst *mem_strObj =
      context->CreateEntryBlockAlloca(F, "currResult", strObjType);
