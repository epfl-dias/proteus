...
ExprMaterializer::ExprMaterializer(expressions::Expression *toMat, int linehint,
  ...
  /* Arbitrary initial buffer sizes */
  /* No realloc should be required with these sizes for synthetic large-scale
   * numbers */
  //    size_t sizeBuff = 10000000000;
...
void ExprMaterializer::consume(Context *const context,
  ...
  Value *val_exprSize =
      context->createInt64(context->getSizeOf(val_toMat->getType()));
   ...
   */
  {
    ...
    *(info.itemCount) = 0;
    ...
    Value *mem_itemCount =
        context->CastPtrToLlvmPtr(int64PtrType, (void *)info.itemCount);
