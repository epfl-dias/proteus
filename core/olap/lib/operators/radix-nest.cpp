...
map<RecordAttribute, ProteusValueMemory> *Nest::reconstructResults(
...

  {
    ...
    //         tuplesIdentifiers.begin(); for (; it != tuplesIdentifiers.end();
    //         it++) {
    ...
    for (const auto &expr2 : mat.getWantedExpressions()) {
      ...
      // cout << "Dealing with " << expr2->getRegisteredAs().getRelationName()
      // << "_"
      ...
      AllocaInst *mem_field = context->CreateEntryBlockAlloca(
          F, "mem_" + currField, payloadType->getElementType(i));
      vector<Value *> idxList = vector<Value *>();
      idxList.push_back(context->createInt32(0));
      idxList.push_back(context->createInt32(i));
...
void Nest::probeHT() const {
...

  {
                           ...
                           &rLoopCond, &rLoopBody, &rLoopInc, &rLoopEnd);
    {
      ...
      //            Value *htRshiftedPtr_j =
      //            Builder->CreateInBoundsGEP(htRshiftedPtr,
      //                    val_j);
      //            Value *val_key_r_j = context->getStructElem(htRshiftedPtr_j,
      //            0); Value *val_idx = val_key_r_j;
      ...
      map<RecordAttribute, ProteusValueMemory> *retrievedBindings;
      {
                               ...
                               &ifKeyMatch, hitLoopInc);
        {
          ...
          for (const auto &k : f_grouping_vec) {
            // std::cout << "===> " << k->getRegisteredAs().getRelationName() <<
            // " " << k->getRegisteredAs().getAttrName() <<std::endl; for (const
            ...
            //                RecordAttribute
            //                att{f_grouping->getRegisteredAs()};
            //                expressions::InputArgument
            //                arg{att.getOriginalType(), -1,
            //                list<RecordAttribute>{att}};
            //                expressions::RecordProjection   key{&arg, att};

            // std::cout << "=1?" <<
            // k->getRegisteredAs().getOriginalRelationName() << std::endl;
            // std::cout << "=2?" << k->getRegisteredAs().getRelationName() <<
            // std::endl; std::cout << "=3?" <<
...
//                    }
          ...
          for (; itAcc != accs.end();
               itAcc++, itExpr++, itMem++,
               itLabels++) {  // increment only when using materialized results:
            Monoid acc = *itAcc;
            expression_t outputExpr = *itExpr;
      ...
      */
      ...
      BasicBlock *nextUnmarkedCond, *nextUnmarkedLoopBody, *nextUnmarkedLoopInc,
          *nextUnmarkedLoopEnd;
      context->CreateForLoop("nextUnmarkedCond", "nextUnmarkedBody",
                             "nextUnmarkedInc", "nextUnmarkedEnd",
                             &nextUnmarkedCond, &nextUnmarkedLoopBody,
      ...
      Function *func_releaseMemory = context->getFunction("releaseMemoryChunk");
      ArgsV.clear();
