...
import ch.epfl.dias.calcite.adapter.pelago.traits.{RelComputeDevice, RelComputeDeviceTraitDef, RelDeviceType, RelDeviceTypeTraitDef, RelHetDistribution, RelHomDistribution, RelHomDistributionTraitDef, RelPacking}
...
//    val rf = 1
  ...
  override def implement(target: RelDeviceType, alias: String): (Binding, JValue) = {
    ...
    val cond = emitExpression(RexUtil.expandSearch(getCluster.getRexBuilder, null, getCondition), List(childBinding), this)
