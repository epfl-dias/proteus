...
object CostModel {
  ...
  def getDevCount(traitSet: RelTraitSet): Double =
    if (
      traitSet.containsIfApplicable(RelHomDistribution.RANDOM) || traitSet
        .containsIfApplicable(RelHomDistribution.BRDCST)
    ) {
...
//  private val uns = new mutable.HashMap[PelagoRel, mutable.Set[Long]]()
  ...
  def getNonCumulativeCost(rel: PelagoRel): Cost =
    rel match {
      // Converters
...
//      if (splits.nonEmpty) return ReallyInfiniteCost()
        MemBW(
          rel.getRowType.getFieldCount * blockSize * 10 /* 10 for sync cost */
        ) +
          InterconnectBW(
            rel.getRowType.getFieldCount * 8
          ) // * rel.getCluster.getMetadataQuery.getRowCount(rel.getInput(0)))
      case router: PelagoSplit =>
        MemBW(rel.getRowType.getFieldCount * 10 /* 10 for sync cost */ )
      case router: PelagoRouter
          if router.getHomDistribution == RelHomDistribution.BRDCST && router.getTraitSet
            .containsIfApplicable(RelPacking.UnPckd) =>
        InfiniteCost()
      case router: PelagoRouter =>
        MemBW(
          router.getRowType.getFieldCount * 10 /* 10 for sync cost */
        ) + // FIXME: or 100?
          InterconnectBW(router.getRowType.getFieldCount * 4 * {
            if (
              ...
              router.getTraitSet.containsIfApplicable(RelPacking.Packed)
            ) {
              blockSize
            } else {
      ...
      case cross: PelagoDeviceCross
          if cross.getDeviceType() == RelDeviceType.NVPTX && cross.getTraitSet
            .containsIfApplicable(RelPacking.UnPckd) =>
        InfiniteCost()
      case cross: PelagoDeviceCross =>
        MemBW(
          cross.getRowType.getFieldCount * 10 /* 10 for sync cost */
        ) + // FIXME: or 100?
          InterconnectBW((cross.getRowType.getFieldCount) * 8 * {
            if (
              ...
              cross.getTraitSet.containsIfApplicable(RelPacking.Packed)
            ) {
              blockSize
            } else {
              blockSize * 0.2 //FIXME: patch until we implement exclicit mem-moves in the plans
      ...
      case _: PelagoFilter =>
        Compute(16)
      case agg: PelagoAggregate
          if agg.getAggCallList.asScala.exists(
            _.getAggregation.getKind == SqlKind.AVG
          ) =>
        InfiniteCost()
      case red: PelagoAggregate if red.getGroupCount == 0 =>
        Compute(16) + MemBW(
          0.01 * (8 / blockSize) * red.getRowType.getFieldCount
        )
      case agg: PelagoAggregate if agg.getGroupCount > 0 =>
        Compute(16) + RandomMemBW(
          agg.getRowType.getFieldCount + 0.01 * agg.getInput.getRowType.getFieldCount * 10000000
        )
      case project: PelagoProject =>
        Compute(
          project.getRowType.getFieldCount * 16 /* naive estimation for number of instr / expr */
        ) +
          MemBW(1)
      case _: PelagoSort =>
        MemBW(8)
  ...
  def getNonCumulativeCost(rel: PelagoToEnumerableConverter): Cost =
    MemBW(rel.getRowType.getFieldCount * 64 * 1024)

  def getNonCumulativeCost(rel: Join): Cost = {
    if (rel.getCondition.isAlwaysTrue || !rel.analyzeCondition().isEqui)
      return InfiniteCost()
...
//    } else {
    ...
    val buildTupleBytes = (build.getRowType.getFieldCount * 8 + 2) * Math.log(
      rel.getCluster.getMetadataQuery.getRowCount(build)
    )
...
//    }
  ...
  def getNonCumulativeCost(rel: RelNode): Cost =
    rel match {
