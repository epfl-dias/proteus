...
/**
  * Implementation of [[org.apache.calcite.rel.core.Project]]
  * relational expression in Pelago.
  */
class PelagoProject protected (
    ...
    extends Project(cluster, traitSet, hints, input, projects, rowType)
    with PelagoRel {
//  assert(getConvention eq PelagoRel.CONVENTION())
  ...
  override def implement(
    ...
    val exprsJS: JValue = getNamedProjects.asScala
      .map { e =>
        (RexUtil.expandSearch(getCluster.getRexBuilder, null, e.left), e.right)
      }
      .map { e =>
        emitExpression(e._1, List(childBinding), this)
          .asInstanceOf[JsonAST.JObject] ~ ("register_as", (
          "attrName",
          e._2
  ...
  def create(
    ...
    val traitSet = input.getTraitSet
      ...
      .replaceIf(
        ...
        () => PelagoRelMdHomDistribution.project(mq, input, projects)
      )
      .replaceIf(
