...
//        assert !condition.isAlwaysTrue();
  ...
  override def computeBaseSelfCost(
    ...
    if (condition.isAlwaysTrue || !analyzeCondition.isEqui)
...
//  override def estimateRowCount(mq: RelMetadataQuery): Double = mq.getRowCount(getRight) * mq.getPercentageOriginalRows(getLeft);//Math.max(mq.getRowCount(getLeft), mq.getRowCount(getRight))
  ...
  private def getTypeSize(t: RelDataType) =
  ...
  def getCompositeKeyExpr(
      operands: ImmutableList[RexNode],
      arg: Int,
      bindings: List[Binding],
      alias: String
  ): (JObject, RexInputRef, Int) = {
    var size = 0
    var maxsize = 0
    (
      ("expression", "recordConstruction") ~ ("type", (
        "type",
        "record"
      )) ~ ("attributes",
      operands.asScala.zipWithIndex
        .map(p => {
          assert(
            p._1.isA(SqlKind.EQUALS),
            "Only equality hash joins supported, found: " + getCondition
          )
          var op = p._1.asInstanceOf[RexCall].operands.get(arg)
          val op_other = p._1.asInstanceOf[RexCall].operands.get(1 - arg)
          if (
            op.isInstanceOf[RexInputRef] && op_other.isInstanceOf[RexInputRef]
          ) {
          ...
          val name =
            if (op.isInstanceOf[RexInputRef])
              op.asInstanceOf[RexInputRef].getName
            else p._2.toString
          val je =
            emitExpression(op, bindings, this).asInstanceOf[JsonAST.JObject]
          (
            "name",
            name
          ) ~ ("e", je ~ ("register_as", (
        ...
        .toList),
      new RexInputRef(
        bindings.map(e => e.fields.length).sum + arg,
  ...
  def getKeyExpr(
    ...
    assert(
      cond.isA(SqlKind.EQUALS) || cond.isA(SqlKind.AND),
      "Only equality hash joins supported, found: " + cond
    )
    ...
    if (cond.isA(SqlKind.EQUALS)) {
      ...
      (
        emitExpression(
          operands.get(if ((arg == 0) == (rt < ro)) arg else (1 - arg)),
          bindings,
          this
  ...
  override def implement(
    ...
    val (probe_k, probe_keyRexInputRef, _) = getKeyExpr(
      getCondition,
      1,
      List(build_binding, probe_binding),
      alias.getPelagoRelName
    )
    val (build_k, build_keyRexInputRef, _) = getKeyExpr(
      getCondition,
      0,
      List(build_binding, probe_binding),
      alias.getPelagoRelName
    ...
    val json = op ~
      ("build_k", build_k ~ ("register_as", (
        "attrName",
        build_keyName
      ) ~ ("relName", alias.getPelagoRelName))) ~
      ("build_input", build_child) ~
      ("probe_k", probe_k ~ ("register_as", (
        "attrName",
        probe_keyName
      ) ~ ("relName", alias.getPelagoRelName))) ~
      ("hash_bits", hash_bits) ~
    ...
    val binding: Binding =
      Binding(alias, build_binding.fields ++ probe_binding.fields)
...
//    assert(info.isEqui, "should had been equi-join!")
    ...
    val dev = List(left, right).map((e) =>
      mq.asInstanceOf[PelagoRelMetadataQuery].computeType(e)
    )
    val traitSet = right.getTraitSet
      ...
      .replaceIf(
        ...
        () =>
          RelComputeDevice.from(
            List(
              RelComputeDevice.from(dev),
              RelComputeDevice.from(left),
              RelComputeDevice.from(right)
            )
          )
      )
      .replaceIf(
