...
import ch.epfl.dias.calcite.adapter.pelago.schema.PelagoResultTable
...
object PelagoExecutor {
  ...
  var builder =
    if (Repl.isMockRun) null
    else new ProcessBuilder(Repl.executor_server).redirectErrorStream(true)
    ...
    if (Repl.isMockRun) null
      ...
      new java.io.PrintWriter(
        (new java.io.OutputStreamWriter(
          new java.io.BufferedOutputStream(process.getOutputStream())
        )),
        true
      )
  var stdoutReader =
  ...
  def getEnumerableResult(root: DataContext, label: String): Enumerable[_] = {
    if (rowType == null) {
      ...
      val path = run(
        "execute plan from statement " + label,
        "execute statement",
        label,
        null
      )

      new PelagoResultTable(Sources.of(new File(path)), rowType, false)
        .scan(root)
    }
  }

  def restartEngine(): Unit =
    this.synchronized {
      if (Repl.isMockRun) return
      if (process == null || !process.isAlive) {
        builder =
          new ProcessBuilder(Repl.executor_server).redirectErrorStream(true)
        process = builder.start()
        loadedfiles = null
        PelagoToEnumerableConverter.preparedStatementsCache.clear()

        stdinWriter = new java.io.PrintWriter(
          (new java.io.OutputStreamWriter(
            new java.io.BufferedOutputStream(process.getOutputStream())
          )),
          true
        )
        stdoutReader = new java.io.BufferedReader(
          new java.io.InputStreamReader(process.getInputStream())
        )
      }
    }
  ...
  def run(
    ...
    this.synchronized {
      ...
      if (
        loadedfiles == null || files == null || !files.subsetOf(loadedfiles)
      ) {
      ...
      && !line.startsWith("prepared statement with label ")) {
        ...
        if (line.contains("Optimization time: ")) {
          tcodeopt = tcodeopt + java.lang.Long
            .parseLong(" (\\d+)ms$".r.findFirstMatchIn(line).get.group(1))
        }
      ...
      if (line == null) {
        System.out.println("waiting pelago engine exiting")
        process.waitFor(1, TimeUnit.SECONDS);
        assert(!process.isAlive)
        restartEngine()
        throw new RuntimeException("Internal Engine Failure")
      }
      ...
      if (Repl.timings) {
        ...
        TimeKeeper.INSTANCE.refreshTable(
          path,
          cmd_type,
          if (Repl.isHybrid) "hybrid"
          else if (Repl.isCpuonly) "cpuonly"
          else "gpuonly",
          plan,
          query_name
        )
